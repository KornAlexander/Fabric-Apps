---
order: 8
app: games-and-learn/doom
slug: doom
galleryPost: docs/gallery-posts/doom.md
galleryUrl:            # paste the live gallery URL here in Phase 5a, then it replaces {{galleryUrl}} below
status: draft
language: en
assetsReady: false
assets:
  - docs/previews/doom.webp
  - games-and-learn/doom/media/doom-demo.gif
---

# 08 — doom

*Post 8 of 15 in the Fabric Apps series. One per weekday.*

## Post text

```text
But can it run Doom?

Yes. Shareware DOOM boots in the browser through js-dos, wrapped in a Fabric-authenticated app, with scores written to a Fabric SQL database.

Credit where it is due: the original concept and the Fabric integration are Sander van de Velde's. This is a port of his idea into the gallery, with his name on it.

The traditional proof that a platform has matured. Fabric passes.

Source, and always the latest version:
https://github.com/KornAlexander/Fabric-Apps/tree/main/games-and-learn/doom

Gallery entry: {{galleryUrl}}

#MicrosoftFabric #PowerBI #FabricApps #Doom #FabricSQL
```

## Notes for the composer

- Attach: `games-and-learn/doom/media/doom-demo.gif` (or the MP4 if LinkedIn handles it better on the day).
- Paste as plain text; LinkedIn strips markdown. The blank lines are the formatting.
- Mentions are added by hand in the composer, not here.
- ⚠️ Tag Sander van de Velde by hand — the credit is the point of the post.
