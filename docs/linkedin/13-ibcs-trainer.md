---
order: 13
app: games-and-learn/ibcs-trainer
slug: ibcs-trainer
galleryPost: docs/gallery-posts/ibcs-trainer.md
galleryUrl:            # paste the live gallery URL here in Phase 5a, then it replaces {{galleryUrl}} below
status: draft
language: en
assetsReady: false
assets:
  - docs/previews/ibcs-trainer.webp
  - games-and-learn/ibcs-trainer/media/ibcs-trainer-demo.gif
---

# 13 — ibcs-trainer

*Post 13 of 15 in the Fabric Apps series. One per weekday.*

## Post text

```text
You learn a charting standard faster by destroying bad charts than by reading the rules.

IBCS Trainer is a platformer where each level teaches one IBCS notation rule. Pie charts explode. The wrong chart for a time series has to go.

It is the same standard you would otherwise learn from a 200-page manual, in the order you would actually meet the problems.

Scores persist to Fabric SQL, so a team can run it as a shared exercise.

Source, and always the latest version:
https://github.com/KornAlexander/Fabric-Apps/tree/main/games-and-learn/ibcs-trainer

Gallery entry: {{galleryUrl}}

#MicrosoftFabric #PowerBI #FabricApps #IBCS #DataViz #Training
```

## Notes for the composer

- Attach: `games-and-learn/ibcs-trainer/media/ibcs-trainer-demo.gif` (or the MP4 if LinkedIn handles it better on the day).
- Paste as plain text; LinkedIn strips markdown. The blank lines are the formatting.
- Mentions are added by hand in the composer, not here.
