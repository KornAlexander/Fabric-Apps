...
/**
 * Read scopes used by the availability probes (PLAN.md §8.2).
 *
 * `User.Read` is consented by default for any signed-in user, so a probe can
 * always reach T0. `Directory.Read.All` and `Group.Read.All` are what lift the
 * Entra module to T1, and their absence is reported honestly rather than
 * silently swallowed.
 */
...
/** Minimum scope set that every signed-in user already has. */
export const GRAPH_MINIMAL_SCOPES = ['https://graph.microsoft.com/User.Read'];
