...
**The airspace above Oberstdorf and the Nebelhorn as an interactive 3D flight map.**
...
3. **Watch who is flying right now.** *(Phase 4.)* Live FANET/FLARM traffic when the sky cooperates.
4. **Ask it questions.** *(Phase 6.)* Voice and chat over the same data.
...
The framing and content rules are binding and live in **PLAN.md §2 — they outrank every
...
| `npm run curate` |  |
| `npm run data:build` | download and derive the input data |
