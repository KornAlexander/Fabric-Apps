...
![Campus Twin](../../../docs/previews/campus-scheduler.webp)
...
**There is a narrated film of this**, recorded against the deployed app rather than a dev server:
`media/campus-insights-demo.mp4`, with an eight-second opener joined on the front. It is not in the
repository — rendered video is reproducible output, so `media/*.mp4` is gitignored — and
The films are rebuilt from the app itself; the recording tooling is not part of the public tree.

⚠️ The opener is **generated imagery and not the twin** — see
[What is real and what is not](#what-is-real-and-what-is-not). Everything after the cut is the app.
...
⚠️ **One file in this repository is generated imagery, and it is not the twin.**
`media/campus-intro.mp4` is an eight-second cinematic opener made with Sora 2 — a photoreal aerial
over a German university campus that does not exist. It is a mood opener for the front of a demo
reel and nothing else. Every claim the table above makes is about the *app*; this clip is not the
app, must never be shown as what the app renders, and would undermine exactly the property this
section exists to protect if it were. The recorded demos beside it
(`campus-insights-*.mp4`, `campus-scheduler-lmu-guide.mp4`) are screen captures of the real thing.
...
Campus Twin turns a university's timetable into something you can see. Every teaching
session is placed in a real room in a photoreal 3D model of the campus, built from official
state survey data, so an over-full lecture hall and a building nobody is using are visible
rather than inferred from a table. A planner can move a session and watch the consequences
land: room utilisation, walking distance between consecutive sessions, and staffing load.

Eleven sites are configured. One of them, `campus-demo`, is an invented university used for
demonstrations, and it is the only one whose film ships with this repository.

## Fabric architecture

- **Rayfin app** with Fabric authentication, so a planner signs in once and the app queries
  as that user.
- **Fabric SQL database** for the plan itself: proposals, confirmations and per-lecturer
  availability. The tables hold statements somebody made, never a cross join of defaults.
- **Semantic model and report** over the same data, generated by `tools/fabric/`, so the
  numbers in the app and the numbers in Power BI cannot drift apart.
- **Static hosting** for the client, with terrain served separately because eleven sites of
  survey data do not fit inside a 100 MB package.

## Data sources

| What | Where it comes from | Licence |
|---|---|---|
| Terrain, orthophoto, buildings, trees | Official state survey open data (LDBV Bavaria, LGL Baden-Wuerttemberg, Geobasis NRW) | CC BY 4.0, per-state attribution in `NOTICE.md` |
| Building footprints and room polygons | OpenStreetMap | ODbL |
| Elevation | Copernicus WorldDEM-30 | per its own terms |
| Timetables | Generated from each site's academic profile by `tools/data/generate_timetable.py` | ours |

⚠️ **No university's own timetable export is included.** `config/release.json` is a switch,
read by both the app and the Python pipeline, that decides what a build may carry. In this
tree it stands at `realCustomerData: exclude` and `navigatumData: synthetic`, which means
every timetable here is invented and reproducible from configuration. `npm run
check:release` and `python tools/verify_publishable.py` both fail if that stops being true.

## Credits

Terrain, imagery and building data are published by the German state survey authorities and
by OpenStreetMap contributors; the exact attribution strings they require are reproduced
verbatim in `NOTICE.md` and must not be reworded. Copernicus WorldDEM-30 is used under its
own terms. Everything else here is mine, and it is a personal project, not a Microsoft
