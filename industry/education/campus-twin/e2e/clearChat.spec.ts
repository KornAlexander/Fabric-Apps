...
/**
 * The assistant stream, matched by PATH rather than by a URL glob.
 *
 * ⚠️ IT WAS `'**\/api/assistant/stream'`, AND A QUERY STRING SILENTLY UNMATCHED IT. `apiUrl` was
 * changed to name the university on every backend request — rightly: without it the assistant
 * answered questions about one university out of another one's plan, confidently and in German —
 * so the URL became `.../api/assistant/stream?site=oth`, and a Playwright glob matches the WHOLE
 * url, query included. The stub then never fired, the request reached the real backend, and the
 * test failed on the answer text rather than on anything to do with clearing a chat.
 *
 * A predicate on `pathname` is immune to that, and to whatever `apiBase()` resolves to.
 */
const ASSISTANT_STREAM = (url: URL) => url.pathname.endsWith('/api/assistant/stream');

test('the clear button is not offered until there is something to clear', async ({ page }) => {
  await page.route(ASSISTANT_STREAM, (route) =>
...
test('an answer discarded mid-flight never lands in the next question', async ({ page }) => {
  ...
  let call = 0;
  await page.route(ASSISTANT_STREAM, async (route) => {
    call += 1;
    if (call === 1) {
