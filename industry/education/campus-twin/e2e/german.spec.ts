...
test('the guard does not fail open while the backend site is still unknown', async ({ page }) => {
  ...
  //
  // ⚠️ This used to look for /geladen|loading/i, and that was the SEED rather than the BEHAVIOUR.
  // The drawer now answers the same question far better — it names the cold start and says it can
  // take up to two minutes — so the generic in-grid "wird geladen" no longer fires in this state.
  // The property being guarded is unchanged: something has to explain the wait. Asserting the
  // notice itself keeps the teeth — a regression to a blank grid still fails here.
  const waking = page.getByTestId('calendar-panel').getByTestId('planner-waking');
  await expect(waking).toBeVisible();
  expect((await waking.innerText()).trim().length).toBeGreaterThan(20);
});
