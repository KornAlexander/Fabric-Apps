...
async function settled(locator: ReturnType<Page['getByTestId']>) {
  /*
    ⚠️ BOUNDED READS, BECAUSE THE SPOTLIGHT CAN GO AWAY MID-MEASUREMENT. Not every step has a
    target, so `guide-spotlight` unmounts between steps — and `boundingBox()` with no timeout WAITS
    for a missing element, so the read did not return null, it hung until the 90-second test
    timeout. The call log said it plainly: "waiting for getByTestId('guide-spotlight')".

    The caller checks `isVisible()` first, which is exactly the race: true while the previous
    step's spotlight is still fading, gone by the time it is measured. A vanished spotlight is not
    on the university switcher, so there is nothing to measure and `null` is the honest answer.
  */
  const read = () => locator.boundingBox({ timeout: 1_000 }).catch(() => null);
  let last = await read();
  for (let i = 0; i < 20; i += 1) {
    await locator.page().waitForTimeout(50);
    const next = await read();
    if (next === null) return null;
    if (last && next.x === last.x && next.y === last.y) return next;
    last = next;
  }
  return last;
}
...
test('a campus without a timetable is not walked through one', async ({ page }) => {
  // ⚠️ THE SITE IS DERIVED, NOT NAMED. This said "Garching and Tübingen have no planner" and drove
  // `/?aoi=tuebingen`; Tübingen was later given a generated timetable, so the tour correctly began
  // telling it about its week and this test failed for being out of date rather than for a defect.
  // It is the fourth spec to make that mistake — see `solverlessAoi()`.
  //
  // The property is unchanged: on a site the app withholds the calendar from, a tour step about a
  // week grid would reintroduce exactly the claim the rest of the app refuses to make.
