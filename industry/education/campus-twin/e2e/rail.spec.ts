...
/**
 * A site with no planner behind it — FOUND, not named.
 *
 * The derivation lives in `release.ts` because this is the fourth spec to need it and the first
 * three each kept their own copy of the answer. See `solverlessAoi()` for what went wrong with
 * writing the site's name down.
 */
const TWIN_AOI = solverlessAoi();
const TWIN_SITE = `/?aoi=${TWIN_AOI}`;
...
test.describe('navigation rail', () => {
  ...
  test('a twin with no timetable offers no week, no walks and no changes', async ({ page }) => {
    ...
    // Absent rather than dead. This site has no solver behind it, so these would set state and
    // open nothing at all.
    ...
    /*
     * ⚠️ AND WHAT IT SAYS INSTEAD MUST NOT CONTRADICT THE PANEL UNDER IT. The notice promised
     * "Gebäude, Räume und Analysen sind vorhanden" on a site whose own analysis panel says there
     * are no interiors and that none will be invented — two boxes, one screen, opposite claims.
     * A unit test cannot see this, because the contradiction is between two strings that are each
     * individually fine; it only exists once they are rendered together.
     *
     * Asserted on the DOM rather than on the catalogue for the same reason: the unit guard checks
     * the WORDING, this one checks that the shell still picks the right wording at all.
     */
    const notice = page.getByTestId('no-planner');
    await expect(notice).toBeVisible();
    await expect(notice).not.toContainText('Räume und Analysen sind vorhanden');
    await expect(notice).not.toContainText('rooms and the lenses are here');
  });
});
