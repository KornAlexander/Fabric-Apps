...
for (const site of [
  ...
  {
    aoi: 'tuebingen',
    heading: 'Universität Tübingen',
    lens: 'condition',
    panel: 'condition-panel',
    // 6 413 of 6 417 roofs measured, five wall classes including the whitewash of eleven churches.
    minRoofColours: 1500,
    minWallColours: 4,
    planner: aoiHasPlanner('tuebingen'),
  },
  {
    aoi: 'fau-erlangen',
    heading: 'FAU Erlangen-Nürnberg',
    // ⚠️ THIS PINNED `lens: null` AND SAID SO AT LENGTH: "FAU publishes no timetable and has no
    // indoor survey, so occupancy, staffing and quality are impossible". That was true, and then
    // FAU was given a generated timetable and three lenses, and the note went on explaining a
    // state that had ended while the assertion below demanded the app show no lens card at all.
    //
    // The property it was written for is unchanged and still tested: a site that offers no lens is
    // a correct site, and offering a card that opens nothing is the bug. Which case applies is now
    // read from the config instead of restated here.
    lens: firstLensFor('fau-erlangen'),
    panel: null,
    ...
    minRoofColours: 1500,
    minWallColours: 3,
    planner: aoiHasPlanner('fau-erlangen'),
  },
  {
    aoi: 'koeln',
    heading: 'Universität zu Köln',
    lens: firstLensFor('koeln'),
    panel: null,
    ...
    minRoofColours: 1000,
    minWallColours: 3,
    planner: aoiHasPlanner('koeln'),
  },
  {
    aoi: 'aachen',
    heading: 'RWTH Aachen',
    lens: firstLensFor('aachen'),
    panel: null,
    ...
    minRoofColours: 1200,
    minWallColours: 3,
    planner: aoiHasPlanner('aachen'),
  },
  {
    aoi: 'muenster',
    heading: 'Universität Münster',
    lens: firstLensFor('muenster'),
    panel: null,
    minRoofColours: 1000,
    minWallColours: 3,
    planner: aoiHasPlanner('muenster'),
  },
  /*
   * ⚠️ EXCLUDED SITES ARE FILTERED OUT, NOT LEFT TO FAIL QUIETLY. `config/release.json` drops
   * whole AOIs from the build to stay under the 100 MB static-package ceiling, and `AOIS` honours
   * that — so `?aoi=<excluded>` does not render that campus, it FALLS BACK TO THE DEFAULT SITE.
   *
   * That is the nasty shape: the page loads, the scene renders, no error appears, and every
   * assertion below runs happily against the wrong university. Only the `heading` check would
   * have caught it, six minutes into a spec that is not in the fast lane — which is precisely how
   * the stale pin further down went unnoticed for two plan sections.
   *
   * Reading the exclusion list here keeps the rows above as the description of what a complete
   * portfolio looks like, while testing only what this build actually ships.
   */
] as const).filter((site) => !EXCLUDED_AOIS.includes(site.aoi))) {
  test.describe(`${site.aoi} — a campus twin`, () => {
    ...
    test('still offers the lens its data supports — and it opens', async ({ page }) => {
      ...
      // ⚠️ STATED, NOT ASSUMED — with a convention as the fallback. `lens` and `panel` began as two
      // independent fields that TypeScript cannot correlate, so a row naming a lens and forgetting
      // its panel fell through to a click with nothing checked afterwards. Every lens panel in
      // `src/lenses/*` carries `data-testid="<lens>-panel"`, so a row may now leave `panel` null
      // and get the conventional one; naming it explicitly still wins, and a row that names a lens
      // whose panel does not follow the convention fails on the assertion below rather than
      // silently checking nothing.
      const panel = site.panel ?? `${site.lens}-panel`;
...
test('the switch reaches all nine universities', async ({ page }) => {
  ...
  //
  // ⚠️ AND IT IS PINNED BY HAND ON PURPOSE, unlike the `planner` and `lens` fields in the table
  // above, which are read from the config precisely because they MIRROR it. This one asserts that
  // the UI reaches the registry, so deriving it would compare the registry with itself and pass on
  // an empty menu. `uni-regensburg` was registered in PLAN §76 and this edit was owed then; it was
  // missed because this spec is not in the fast lane.
  //
  // ⚠️ DIESE LISTE WAR SCHON EINMAL ZWEI STANDORTE KURZ, UND ZWAR AUS EINEM GRUND, DEN ES NICHT
  // MEHR GIBT. `fau-erlangen` und `aachen` standen in `config/release.json` unter `excludeAois`,
  // weil das statische Paket bei 100 MB gedeckelt ist und das volle Portfolio nicht mehr darunter
  // passte — gemessen 95,5 MB bei neun Standorten, also 4,5 MB Kopfraum. Seit das Gelände von
  // einem eigenen Terrain-Host kommt und der Prune-Schritt es ganz aus dem Paket wirft, sind es
  // 0,4 MB bei elf Standorten, und beide sind zurück. `excludeAois` ist leer.
  // `pinnedSiteLists.test.ts` vergleicht dieses Array in Millisekunden gegen die Registry — es hat
  // genau diese Änderung sofort gemeldet, statt sie sechs Minuten später im Browser auffliegen zu
  // lassen.
