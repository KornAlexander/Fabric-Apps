...
from tools import TOOL_IMPLEMENTATIONS, detect_conflicts
import rules
...
def health(
    ...
    ⚠️ `site` WAS MISSING FROM THIS SIGNATURE AND EVERY OTHER ROUTE HAD IT, which turned the SITE
    GUARD into the thing it was written to prevent. `backendSite()` in the front end asks THIS
    route which university it is talking to and refuses to render a calendar when the answer does
    not match the AOI. With one container per university the module-level `store` was that answer.
    With one container serving all nine it is the container's DEFAULT — so every site except the
    default was told "the plan data belongs to somewhere else", the calendar and the assistant
    switched themselves off, and the twin looked broken while being perfectly correct underneath.
    Measured on RWTH Aachen against a local multi-site backend that held RWTH's data all along.

    The failure was the safe direction of the guard, which is why it is worth writing down: it
    withheld a real plan rather than showing the wrong one. But a guard that fires on the correct
    case is a guard people turn off.
    """
...
def _conflict_key(c: dict[str, Any]) -> str:
    """A stable identity for a conflict, so "the same problem" is recognised across two checks.

    ⚠️ MIRRORS `conflictKey` in `src/planner/moveCheck.ts`, field for field. If the two drift, the
    browser and the server disagree about which conflicts are NEW, and a planner sees a green
    "konfliktfrei" and then a refusal — or worse, the reverse. `__tests__/moveCheck.test.ts` pins
    the TypeScript half and `test_hand_moves.py` pins this one against the same fixtures.

    ⚠️ THE SESSION LIST MUST BE SORTED. `room_double_booked` reports its pair as
    [incumbent, newcomer], and which is which depends on dictionary order in the placement map — so
    an unchanged pre-existing clash can come back the other way round and would look new every time.
    """
    return "|".join([
        str(c.get("type") or ""),
        str(c.get("slotId") or ""),
        str(c.get("roomId") or ""),
        str(c.get("teacherId") or ""),
        str(c.get("attendeeId") or ""),
        str(c.get("sessionId") or ""),
        "+".join(sorted(c.get("sessions") or [])),
    ])


@app.post("/api/moves/propose")
async def moves_propose(request: Request, x_app_key: str | None = Header(default=None)) -> dict[str, Any]:
    """Turn hand-dragged moves into a proposal, so the confirm gate can accept them.

    ⚠️ WHY THIS EXISTS AT ALL. `/api/draft/apply` takes a `proposalId`, and until now only the
    SOLVER ever registered one — so a move a planner made by hand could be checked and then not
    kept. The gate is NOT widened: this endpoint only mints an id for moves the planner made, and a
    human still confirms the result exactly as they confirm a solver variant.

    ⚠️ THE VERDICT IS RE-COMPUTED HERE, NEVER TRUSTED FROM THE CLIENT. The browser judged these
    moves when they were dragged, and the plan can have moved since — another draft published, an
    availability cell blocked, a room shrunk in the Regelwerk. A stale green is precisely how a
    conflict would get written into a draft with a tick beside it.

    ⚠️ AND THEY ARE CHECKED AS A SET, NOT ONE BY ONE. Two moves that are each legal alone can still
    collide with each other — same room, same hour. Checking them individually, which is what the
    drag bar does, would let that pair straight through.
    """
    _check_key(x_app_key)
    body = await request.json() or {}
    st = _store((body.get("site") or "").strip() or None)

    raw = body.get("moves") or []
    if not isinstance(raw, list) or not raw:
        raise HTTPException(status_code=400, detail="moves is required")

    probe: list[dict[str, Any]] = []
    option_moves: list[dict[str, Any]] = []
    unknown: list[str] = []
    for m in raw:
        session_id = str((m or {}).get("sessionId") or "").strip()
        row = st.assignment_by_session.get(session_id)
        if not row:
            unknown.append(session_id)
            continue
        to_slot = str(m.get("slotId") or row["slotId"])
        to_room = str(m.get("roomId") or row["roomId"])
        if to_slot == row["slotId"] and to_room == row["roomId"]:
            continue  # dragged back where it started — not a change, not an error
        probe.append({"sessionId": session_id, "slotId": to_slot, "roomId": to_room})
        session = st.session_by_id.get(session_id, {})
        course = st.course_by_id.get(session.get("courseId", ""), {})
        option_moves.append({
            "sessionId": session_id,
            "changed": True,  # ⚠️ `proposals.apply` SKIPS a move without this.
            "course": course.get("title") or session.get("courseId"),
            "from": {"slotId": row["slotId"], "roomId": row["roomId"]},
            "to": {"slotId": to_slot, "roomId": to_room},
        })

    # ⚠️ A session the dataset no longer produces is reported, never skipped in silence — the same
    # rule `restore` and `publish` follow. Quietly proposing 2 of 3 moves would be the worst answer.
    if unknown:
        return {"error": "unknown_session", "sessions": unknown}
    if not option_moves:
        return {"error": "no_change"}

    before = detect_conflicts(st)
    after = detect_conflicts(st, moves=probe)
    known = {_conflict_key(c) for c in before.get("detail", [])}
    caused = [c for c in after.get("detail", []) if _conflict_key(c) not in known]
    known_undecidable = {_conflict_key(c) for c in before.get("undecidableDetail", [])}
    new_undecidable = [
        c for c in after.get("undecidableDetail", []) if _conflict_key(c) not in known_undecidable
    ]

    # ⚠️ COUNTS AS WELL AS KEYS. `detail` is truncated at 60 and `undecidableDetail` at 20, so on a
    # plan already at the cap a genuinely new conflict can fall off the end of the list and leave
    # `caused` deceptively empty. The totals cannot be truncated, so they are the backstop.
    grew = after.get("conflicts", 0) > before.get("conflicts", 0)
    grew_undecidable = after.get("undecidable", 0) > before.get("undecidable", 0)

    if caused or grew:
        return {
            "error": "would_conflict",
            "caused": caused[:10],
            "conflictsBefore": before.get("conflicts", 0),
            "conflictsAfter": after.get("conflicts", 0),
        }
    # ⚠️ An undecidable collision is NOT a clean bill. The export cannot say which weeks these run
    # in, so nobody can tell whether this move double-books the room — and "we could not look" must
    # not be written into a draft as though it were "no clash". Same rule as the unchecked counts.
    if new_undecidable or grew_undecidable:
        return {
            "error": "would_be_undecidable",
            "undecidable": new_undecidable[:10],
            "undecidableAfter": after.get("undecidable", 0),
        }

    option = {
        "option": 1,
        "moves": option_moves,
        "sessionsMoved": len(option_moves),
        # ⚠️ NOT A SOLVER RESULT, AND IT MUST NOT CLAIM TO BE. `cost` is the solver's objective and
        # there is none here; `optimalityProven` is rendered in the preview bar, so leaving it true
        # would badge a hand-made move as a proven optimum.
        "cost": None,
        "optimalityProven": False,
        "handMade": True,
    }
    proposal_id = proposals.register(
        [option], question=body.get("label") or "Manuelle Verschiebung", site=st.site
    )
    return proposals.diff(st, proposal_id, 1)
...
@app.get("/api/rules")
def rules_get(site: str = "", x_app_key: str | None = Header(default=None)) -> dict[str, Any]:
    """The rule catalogue the solver obeys — PLAN §26.5, §39.

    ⚠️ Returns the ORDERING plus the derived weights, not raw editable integers (§26.8). The
    weights are shown because a planner is entitled to see what their ranking produced; they are
    not settable, because free weights allow rankings nobody means.
    """
    _check_key(x_app_key)
    return rules.describe((site or "").strip() or None)


@app.post("/api/rules")
async def rules_set(request: Request,
                    x_app_key: str | None = Header(default=None)) -> dict[str, Any]:
    """Change the rules. Refusals are returned per field, not raised.

    ⚠️ NOT GATED, by decision (2026-08-20): anyone may change a rule. That makes the RESPONSE the
    only safeguard, so it says exactly what changed and what was refused rather than answering
    "gespeichert" — see `$durability` for the other half of the honesty.
    """
    _check_key(x_app_key)
    body = await request.json() or {}
    site = (body.get("site") or "").strip() or None
    if body.get("reset"):
        return {"changed": [{"field": "all", "to": "defaults"}], "refused": [],
                "rules": rules.reset(site), "durable": False}
    return {**rules.apply(body, site), "durable": False}


@app.get("/api/rooms")
def rooms_get(site: str = "", schedulable_only: bool = True,
              x_app_key: str | None = Header(default=None)) -> dict[str, Any]:
    """The room stock with its capacities, for the editable table — PLAN §39.3.

    ⚠️ `capacity: null` is a real value and is NOT filtered out. On `oth-real` most rooms have no
    published capacity, and those are precisely the rows a planner would want to fill in; hiding
    them would hide the work.
    """
    _check_key(x_app_key)
    store = _store((site or "").strip() or None)
    rows = [r for r in store.rooms if r.get("schedulable")] if schedulable_only else list(store.rooms)
    booked: dict[str, int] = {}
    for a in store.assignments:
        rid = a.get("roomId")
        if rid:
            booked[rid] = booked.get(rid, 0) + 1
    return {
        "rooms": [
            {
                "roomId": r["roomId"], "roomType": r.get("roomType"),
                "buildingId": r.get("buildingId"), "campusId": r.get("campusId"),
                "capacity": r.get("capacity"),
                "capacityProvenance": r.get("capacityProvenance"),
                "areaM2": r.get("areaM2"),
                "bookedSlots": booked.get(r["roomId"], 0),
            }
            for r in sorted(rows, key=lambda x: str(x.get("roomId")))
        ],
        "unknownCapacity": sum(1 for r in rows if r.get("capacity") is None),
        "$provenanceNote": (
            "Sitzplätze sind überwiegend aus der Fläche abgeleitet, nicht gezählt. "
            "Ein hier eingetragener Wert gilt als vom Planer bestätigt."
        ),
    }


@app.post("/api/rooms/capacity")
async def rooms_set_capacity(request: Request,
                             x_app_key: str | None = Header(default=None)) -> dict[str, Any]:
    """Correct one room's capacity.

    ⚠️ Same contract as `/api/availability`: the answer is not "saved". Shrinking a room below what
    is already booked into it makes the plan illegal, and `nowOverCapacity` names the sessions.
    """
    _check_key(x_app_key)
    body = await request.json() or {}
    store = _store((body.get("site") or "").strip() or None)
    capacity = body.get("capacity")
    result = store.set_room_capacity(
        (body.get("roomId") or "").strip(),
        None if capacity in (None, "") else capacity,
        (body.get("changedBy") or "").strip(),
    )
    return {**result, "durable": False}
