...
def diff(store: ScheduleStore, proposal_id: str, option: int = 1) -> dict[str, Any]:
    ...
    return {
        ...
        # ⚠️ Carried so the UI cannot describe a hand-dragged move as a solver result. The preview
        # bar renders "optimal" or "best in the time available" from `optimalityProven`, and BOTH
        # are false claims about a move a human made by dragging it — nothing was solved, so
        # nothing can be optimal. The recorded verdict string keys on this too.
        "handMade": bool(chosen.get("handMade")),
