...
_SYNTH_DIRS = {
    ...
    # The five twins brought up from terrain-only to a full scheduler. All generated, all named
    # `synthetic-<id>` in the systematic layout — only OTH keeps the unprefixed legacy path.
    "rwth": ROOT / "data" / "synthetic-rwth",
    "koeln": ROOT / "data" / "synthetic-koeln",
    "muenster": ROOT / "data" / "synthetic-muenster",
    "fau": ROOT / "data" / "synthetic-fau",
    "tuebingen": ROOT / "data" / "synthetic-tuebingen",
    "tuberlin": ROOT / "data" / "synthetic-tuberlin",
    # The generic site. Its own week, on the reference build's ground.
    "demo": ROOT / "data" / "synthetic-demo",
}
_SITE_LABELS = {"oth": "OTH Regensburg", "lmu": "LMU München", "tum": "TUM Garching",
                "oth-real": "OTH Regensburg (Echtdaten)",
                "rwth": "RWTH Aachen", "koeln": "Universität zu Köln",
                "muenster": "Universität Münster", "fau": "FAU Erlangen-Nürnberg",
                "tuebingen": "Universität Tübingen", "tuberlin": "TU Berlin",
...
class ScheduleStore:
    ...
    def set_room_capacity(self, room_id: str, capacity: int | None, changed_by: str = "") -> dict:
        """Correct how many people a room holds, and say what that breaks.

        PLAN §39. Seat counts on the generated sites are DERIVED from floor area and a density
        table, and on `oth-real` they are frequently `None` because the Untis export publishes no
        capacities — so "the room actually seats 40" is a correction a planner is better placed to
        make than this pipeline is. This is the surface for it.

        ⚠️ **THE ANSWER IS NOT "SAVED", for the same reason `set_availability`'s is not.** Shrinking
        a room below what is already booked into it does not move anything — it makes the existing
        plan illegal. That is a legitimate thing to record (it is a fact about the building), but a
        UI that replies "gespeichert" and stops has hidden the only consequence that mattered. Every
        write returns the sessions it has just put over capacity.

        ⚠️ **`None` IS A VALUE, NOT A FAILURE.** Clearing a capacity says "nobody has measured this",
        which is different from zero. `detect_conflicts` already counts an unknown capacity as
        `unchecked` rather than passing it — *a check that cannot be performed is not a check that
        passed* — so clearing is how a planner honestly withdraws a number this project guessed.

        ⚠️ **IN-PROCESS ONLY**, exactly like `publish` and `set_availability`.
        """
        room = self.room_by_id.get(room_id)
        if not room:
            return {"error": "room_not_found", "roomId": room_id}
        if capacity is not None:
            try:
                capacity = int(capacity)
            except (TypeError, ValueError):
                return {"error": "capacity_not_a_number", "roomId": room_id, "given": capacity}
            if capacity < 0:
                return {"error": "capacity_negative", "roomId": room_id, "given": capacity}

        before = room.get("capacity")
        room["capacity"] = capacity
        # The provenance has to move with the number, or the app keeps crediting a survey for a
        # figure a planner typed. This is the same rule the room geometry already follows.
        room["capacityProvenance"] = "planner" if capacity is not None else None

        # What this breaks: sessions already assigned to the room that no longer fit.
        broken: list[dict] = []
        if capacity is not None:
            for a in self.assignments:
                if a.get("roomId") != room_id:
                    continue
                sess = self.session_by_id.get(a["sessionId"], {})
                need = sess.get("expectedAttendance")
                if need is not None and need > capacity:
                    broken.append({
                        "sessionId": a["sessionId"], "slotId": a.get("slotId"),
                        "attendees": need, "capacity": capacity,
                    })

        return {
            "roomId": room_id, "from": before, "to": capacity,
            "changedBy": changed_by,
            "nowOverCapacity": broken,
            "$durability": (
                "Die Änderung gilt für diese laufende Instanz. Der Dienst startet nach einer Pause "
                "neu und liest dann wieder die gebauten Werte."
            ),
        }
    ...
    def availability_for(self, teacher: str) -> dict:
        ...
        # invites a planner to treat an assumption as a statement. Only the university's real export sets
