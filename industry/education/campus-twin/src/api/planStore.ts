...
const TENANT_ID = env('VITE_FABRIC_TENANT_ID', 'VITE_TENANT_ID', 'VITE_RAYFIN_TENANT_ID');

/**
 * Where the brokered sign-in popup goes — **with the tenant pinned onto it**.
 *
 * ⚠️ `rayfin env` WRITES THIS URL BARE, AND A BARE URL SIGNS THE WRONG PERSON IN. Fabric's
 * brokered auth opens `<portal>/groups/{ws}/appbackends/{item}/brokeredauth?…`, and with no `ctid`
 * Fabric resolves the tenant from the browser session — i.e. the visitor's HOME tenant. For anyone
 * whose home tenant is not the one this app is deployed to, the popup lands on a cluster where
 * this item does not exist, reports `TOKEN_ACQUISITION_FAILED`, closes itself, and leaves the app
 * sitting on `/auth` with nothing on screen to explain why.
 *
 * ⚠️ AND A LIVE PORTAL SESSION FOR THE RIGHT TENANT DOES NOT SAVE IT. Measured: a browser profile
 * that could already list the deployed workspace still bounced, because the broker reads the
 * identity's home cluster rather than the session in front of it.
 *
 * The tenant is already in the environment, so pinning it costs nothing and removes a failure that
 * only ever shows up for someone signing in from a different tenant — which is every guest, every
 * demo on a partner's machine, and every screenshot taken from a corporate laptop.
 */
const PORTAL_URL = (() => {
  const base =
    env('VITE_FABRIC_PORTAL_URL', 'VITE_PORTAL_URL', 'VITE_RAYFIN_PORTAL_URL')
    || 'https://app.fabric.microsoft.com';
  if (!TENANT_ID || base.includes('ctid=')) return base;
  return `${base}${base.includes('?') ? '&' : '?'}ctid=${TENANT_ID}`;
})();
