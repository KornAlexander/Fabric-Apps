...
/**
 * One backend per university — or one backend for all of them.
 *
 * ⚠️ THIS USED TO SAY "one backend can only ever answer for one university", AND THAT IS NO LONGER
 * TRUE. `SCHEDULER_SITE` is still a single variable on a single container, but it is now only the
 * DEFAULT: `server/schedule_store.py` holds a store per university and every endpoint takes a
 * `?site=` parameter, which `tools/tests/test_multi_site.py` exercises by serving three
 * universities out of one process and refusing a fourth. What has NOT changed is that the front
 * end must still say which university it means — see `apiUrl`, which now names it on every
 * request. Asking for an LMU cohort without saying so returns `not_found`, and asking the
 * assistant returns a confident answer drawn from whichever dataset the container defaults to.
 *
 ...
 *
 * ⚠️ EVERY ENTRY FALLS BACK TO `VITE_SCHEDULER_API_ALL`, and that variable is the whole reason
 * eight universities can ship without eight container apps. A per-site URL still wins where one
 * exists — OTH and LMU have their own containers and their own Fabric datasets, and that stays
 * true — but a site with no container of its own resolves to the shared multi-site one and then
 * identifies itself in the query string. What is NOT here is a fallback to another university's
 * SPECIFIC container: that is the failure this file's history is mostly about.
 */
const MULTI_BASE = env('VITE_SCHEDULER_API_ALL', 'VITE_RAYFIN_SCHEDULER_API_ALL');

const SITE_BASES: Record<string, string> = {
  oth: env('VITE_SCHEDULER_API_OTH', 'VITE_RAYFIN_SCHEDULER_API_OTH') || MULTI_BASE,
  lmu: env('VITE_SCHEDULER_API_LMU', 'VITE_RAYFIN_SCHEDULER_API_LMU') || MULTI_BASE,
  tum: env('VITE_SCHEDULER_API_TUM', 'VITE_RAYFIN_SCHEDULER_API_TUM') || MULTI_BASE,
  // the university's own Untis export, served by its own container beside the generated one.
  'oth-real': env('VITE_SCHEDULER_API_OTHREAL', 'VITE_RAYFIN_SCHEDULER_API_OTHREAL') || MULTI_BASE,
  // The five twins brought up from terrain-only to a full scheduler. None of them has a container
  // of its own and none of them needs one: their datasets ship inside the shared image.
  rwth: env('VITE_SCHEDULER_API_RWTH', 'VITE_RAYFIN_SCHEDULER_API_RWTH') || MULTI_BASE,
  koeln: env('VITE_SCHEDULER_API_KOELN', 'VITE_RAYFIN_SCHEDULER_API_KOELN') || MULTI_BASE,
  muenster: env('VITE_SCHEDULER_API_MUENSTER', 'VITE_RAYFIN_SCHEDULER_API_MUENSTER') || MULTI_BASE,
  fau: env('VITE_SCHEDULER_API_FAU', 'VITE_RAYFIN_SCHEDULER_API_FAU') || MULTI_BASE,
  tuebingen: env('VITE_SCHEDULER_API_TUEBINGEN', 'VITE_RAYFIN_SCHEDULER_API_TUEBINGEN') || MULTI_BASE,
  // TU Berlin, the ninth site. Generated timetable, no container of its own — the shared image.
  tuberlin: env('VITE_SCHEDULER_API_TUBERLIN', 'VITE_RAYFIN_SCHEDULER_API_TUBERLIN') || MULTI_BASE,
  // The generic site. Its own dataset, in the same shared image.
  demo: env('VITE_SCHEDULER_API_DEMO', 'VITE_RAYFIN_SCHEDULER_API_DEMO') || MULTI_BASE,
};
...
/**
 * A backend URL with the ACTIVE UNIVERSITY named on it.
 *
 * ⚠️ THE ONE PLACE THAT SAYS WHICH UNIVERSITY A REQUEST IS ABOUT, and it exists because the
 * backend stopped being single-site before the front end noticed. `resolve_site(None)` falls back
 * to the container's `SCHEDULER_SITE`, so a request that names nothing is answered — correctly,
 * successfully, and about whichever dataset that container happens to default to. With one
 * container per university that was harmless; with a shared container behind
 * `VITE_SCHEDULER_API_ALL` it is the silent-wrong-university failure this file's history is
 * mostly about, so the site is no longer left implicit anywhere.
 *
 * Callers may still pass `site` themselves (`findSubstitute` does, because `/api/tools/{name}`
 * needs it in the query string rather than the body). Setting it here first and letting the
 * caller's own params overwrite it keeps that working and makes the two agree by construction.
 *
 * ⚠️ EXPORTED, because two fetches cannot go through the JSON helpers and must not therefore go
 * without a site. `PlannerChat` reads `/api/assistant/stream` with a `ReadableStream` reader and
 * builds its own request; before this it named no site, so the assistant answered questions about
 * RWTH Aachen out of whichever plan the container defaults to — confidently, in German, with real
 * numbers about another university. Anything that builds a backend URL by hand belongs here.
 */
...
/**
 * Did THIS SITE'S `/api/health` call, the one `backendSite()` already made, come back as a
 * genuine failure rather than a slow-but-healthy "I don't know my site"?
 *
 * ⚠️ NOT A SECOND REQUEST — a side effect of the one `backendSite()` already makes and caches.
 * The planner-is-waking UI (TwinShell, CalendarPanel) needs to tell "the container is still cold"
 * from "it answered and something is actually wrong", and `backendSite()`'s own `.catch(() =>
 * null)` had erased that distinction: a definite network error and an ambiguous 200 both ended up
 * as `null`. Recording which one happened, in the same `.then`/`.catch` that already runs, costs
 * nothing extra over the wire and gives the caller the fact without a duplicate warm-up ping —
 * the exact ping this file's own history already warns against adding twice.
 *
 * Read AFTER `backendSite()` has resolved for the current base; before that it is simply not yet
 * known, which is what the caller's own "waking" state already covers.
 */
const siteFailures = new Map<string, boolean>();
...
export function backendSite(): Promise<string | null> {
  ...
  if (!pending) {
    pending = getJson<{ data?: { site?: string } }>('/api/health')
      .then((response) => {
        siteFailures.set(base, false);
        return response.data?.site ?? null;
      })
      // A backend that cannot say which site it serves is treated as unknown rather than as a
      // match — silence is not agreement. But a request that genuinely failed (network drop, 5xx,
      // CORS) is NOT the same silence, so it is recorded before folding into the same `null`.
      .catch((err) => {
        console.warn('backend health check failed', err);
        siteFailures.set(base, true);
        return null;
      });
...
export function backendHealthFailed(): boolean {
  return siteFailures.get(apiBase()) ?? false;
}
...
export interface PlanFacts {
   ...
   *
   * ⚠️ `Ant` IS NOT A NAME AND MUST NOT LOOK LIKE ONE. the university's real export carries short codes
...
export interface ProposalDiff {
  ...
  /** True when a planner dragged this, so the UI must not describe it as a solver result. */
  handMade?: boolean;
...
/** Why a hand-dragged batch was refused. Every refusal names what it found. */
export interface HandMoveRefusal {
  error: 'would_conflict' | 'would_be_undecidable' | 'unknown_session' | 'no_change';
  caused?: PlanConflict[];
  undecidable?: PlanConflict[];
  sessions?: string[];
...
/**
 * Register hand-dragged moves as a proposal, so the confirm gate can accept them.
 *
 * ⚠️ THE SERVER RE-CHECKS, AND MAY SAY NO. The browser judged each move when it was dragged, but
 * the plan can have moved since — another draft published, an availability cell blocked, a room
 * shrunk in the Regelwerk. It also checks the moves TOGETHER: two that are each legal alone can
 * still collide with each other, which a per-drag check cannot see. A refusal here is the system
 * working, not a bug, and it must be shown rather than swallowed.
 *
 * ⚠️ Writes NOTHING by itself. It mints a `proposalId`; the human still confirms it into a draft
 * through the same gate a solver variant goes through.
 */
export const proposeHandMoves = (
  site: string,
  moves: { sessionId: string; slotId?: string; roomId?: string }[],
  label?: string
) => postJson<ProposalDiff & HandMoveRefusal>('/api/moves/propose', { site, moves, label });
...
/**
 * Replan around a ROOM closing for a day: renovation, water damage, dead projector, a room let to
 * somebody else. The same two questions and the same confirm gate as `replanWithout`.
 *
 * ⚠️ THIS IS NOT A WORKAROUND FOR THE LECTURER REFUSAL, it is an ordinary planning event that
 * happens to need no person at all. That is why it is the only door open at a site whose lecturer
 * attribution is invented (Garching): the room, its capacity and its bookings are surveyed and
 * real there, so every number in the answer is one we are entitled to state.
 */
export async function replanWithoutRoom(
  room: string,
  day: string
): Promise<{ affected: AffectedSessions; proposal: ProposalDiff | null }> {
  const affected = await postJson<AffectedSessions>('/api/tools/get_affected_sessions', {
...
// ── who else could take a session (server/tools.py::find_substitute, PLAN §40) ─────────────

/** One ranked reason. `code` is stable; `text` is the German the ASSISTANT speaks, not the UI. */
export interface SubstituteReason {
  code: 'load' | 'deputatHeadroom' | 'deputatFull' | 'moduleOwner';
  value: number | boolean;
  /** Present only on the two Deputat reasons — the numbers behind the sentence. */
  taught?: number;
  contract?: number;
  text: string;
}
...
export interface SubstituteResult {
  ...
  slotId: string;
  course: string;
  /** May be null — not every source publishes SWS, and then no load projection may be drawn. */
  sessionSws: number | null;
  incumbent: { id: string; name: string | null };
  candidates: SubstituteCandidate[];
  consideredCount: number;
  /** Stable keys for the UI. `rejected` (German prose) is for the assistant and is not read here. */
  rejectedByCode: Partial<Record<'unavailable' | 'alreadyTeaching' | 'travelTooFar', number>>;
  error?: string;
}

/**
 * Who could teach this session instead — ranked, feasible-only.
 *
 * ⚠️ READS, MOVES NOTHING. Assigning a substitute is not a supported change: the whole mutation
 * path (`proposals.apply`) writes `slotId` and `roomId`, and nothing in the server ever sets
 * `teacherId`. So this panel names people and stops there, deliberately — a button that appeared
 * to reassign someone and did not would be worse than no button. Making it real is a separate
 * decision (PLAN §40.6), because it opens a new mutation surface and a schema change.
 *
 * ⚠️ `site` rides in the QUERY STRING. `/api/tools/{name}` reads it from there and splats the JSON
 * body into the tool as keyword arguments — putting `site` in the body would reach
 * `find_substitute(store, site=…)` and come back as `bad_arguments`.
 */
export const findSubstitute = (site: string, sessionId: string) =>
  postJson<SubstituteResult>(
    `/api/tools/find_substitute?site=${encodeURIComponent(site)}`,
