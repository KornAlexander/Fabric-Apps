...
export function AvailabilityPanel({ site, teacher, onChanged }: AvailabilityPanelProps) {
  ...
  /**
   * Substitute lookups, keyed by session — one clash may be expanded while another loads.
   * `null` means "asked, and the answer was an error", which is a different thing from absent.
   */
  const [subs, setSubs] = useState<Record<string, SubstituteResult | null>>({});
  ...
  /**
   * Ask who else could take a session this block just made illegal.
   *
   * ⚠️ Failure is a VALUE, never a throw. A broken lookup must not take the availability editor
   * down with it — the block itself succeeded and the clash list is still the important half.
   */
  const lookUpSubstitute = useCallback(
    async (sessionId: string) => {
      if (subBusy) return;
      setSubBusy(sessionId);
      try {
        const r = await findSubstitute(site, sessionId);
        // ⚠️ KEEP THE REFUSAL, DO NOT FLATTEN IT TO null. The tool reports its refusals in-band with
        // HTTP 200, and they are not all the same thing: on TUM `teacher_not_published` means the
        // export names no lecturer for a booking, so no answer CAN exist — which is a sentence a
        // planner can act on. Collapsing that into "the lookup failed" would report a deliberate,
        // correct refusal as a malfunction.
        setSubs((p) => ({ ...p, [sessionId]: r }));
      } catch {
        setSubs((p) => ({ ...p, [sessionId]: null }));
      } finally {
        setSubBusy(null);
      }
    },
      ...
      */}
      {clashes.length > 0 && (
        <div data-testid="availability-clashes" className="rounded border border-red-500/40 bg-red-500/10 p-2">
          ...
          <ul className="mt-1 space-y-0.5">
            {clashes.map((c) => (
              <li key={c.sessionId} className="text-[0.68rem] text-stone-300">
                <div className="flex items-baseline gap-2">
                  <span className="font-mono text-red-300">{c.slotId}</span>
                  <span className="truncate">{c.course}</span>
                  <button
                    type="button"
                    data-testid={`substitute-find-${c.sessionId}`}
                    disabled={subBusy !== null}
                    onClick={() => void lookUpSubstitute(c.sessionId)}
                    className="ml-auto shrink-0 rounded border border-stone-600 px-1.5 py-0.5 text-[0.62rem] text-stone-200 hover:bg-stone-700 disabled:opacity-40"
                  >
                    {subBusy === c.sessionId
                      ? t('availability.substituteSearching')
                      : t('availability.substituteFind')}
                  </button>
                </div>
                {c.sessionId in subs && (
                  <SubstituteList
                    result={subs[c.sessionId]}
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-stone-800 pt-2">
        {/*
          ...
          failed to load this button silently produced a 414-row workbook for a single
          person. That is how a wrong `site` (the panel was asking `oth-real` for an `oth`
...
/**
 * The ranked substitutes for one clashing session.
 *
 * ⚠️ EVERY WORD COMES FROM THE CATALOGUE, NOT FROM THE SERVER. `find_substitute` also returns a
 * German `text` per reason and a German `$emptyNote`, and both are there for the ASSISTANT, which
 * speaks. Rendering them here would put server-written prose on screen in an English build — the
 * leak this project has already shipped once. So the panel reads `code`, `taught`, `contract` and
 * `rejectedByCode`, all of which are stable identifiers, and writes its own sentences.
 */
function SubstituteList({
  ...
  result: SubstituteResult | null;
  sessionId: string;
  t: (key: string, vars?: Record<string, string>) => string;
}) {
  if (!result) {
    return (
      <p data-testid={`substitute-error-${sessionId}`} className="mt-1 pl-4 text-[0.65rem] text-stone-400">
        {t('availability.substituteFailed')}
      </p>
    );
  }

  // ⚠️ A REFUSAL IS NOT A FAILURE, AND THE DIFFERENCE IS THE WHOLE POINT ON TUM. TUMonline
  // publishes no lecturer per booking, so the people in that dataset are invented and every
  // teacher-shaped question is refused on purpose. Saying "lookup failed" there would invite
  // someone to retry a thing that cannot work, and would hide a data fact worth knowing.
  if (result.error) {
    return (
      <p data-testid={`substitute-refused-${sessionId}`} className="mt-1 pl-4 text-[0.65rem] text-amber-300/90">
        {t(`substitute.err.${result.error}`)}
      </p>
    );
  }

  if (!result.candidates.length) {
    // ⚠️ An empty list is an answer and has to say why, or it reads as "the lookup is broken".
    const why = (['unavailable', 'alreadyTeaching', 'travelTooFar'] as const)
      .map((code) => ({ code, n: result.rejectedByCode[code] ?? 0 }))
      .filter((r) => r.n > 0)
      .map((r) => t(`substitute.rej.${r.code}`, { count: String(r.n) }));
    return (
      <div data-testid={`substitute-empty-${sessionId}`} className="mt-1 pl-4 text-[0.65rem] text-stone-400">
        <p>{t('substitute.none')}</p>
        {why.length > 0 && <p className="text-stone-500">{why.join(' · ')}</p>}
      </div>
    );
  }

  return (
    <div data-testid={`substitute-list-${sessionId}`} className="mt-1 space-y-1 pl-4">
      {result.candidates.map((c) => {
        const dep = c.reasons.find((r) => r.code === 'deputatHeadroom' || r.code === 'deputatFull');
        // ⚠️ THE PROJECTION IS ONLY DRAWN WHEN IT CAN BE COMPUTED. Without the session's SWS there
        // is no honest "afterwards" — and a load figure nobody measured is exactly the kind of
        // confident number this app refuses to invent elsewhere.
        const canProject = dep?.taught !== undefined && dep?.contract !== undefined
          && result.sessionSws !== null && result.sessionSws !== undefined;
        const after = canProject ? dep!.taught! + result.sessionSws! : null;
        const wouldExceed = after !== null && dep?.contract !== undefined && after > dep.contract;

        return (
          <div
            key={c.teacherId}
            data-testid={`substitute-candidate-${c.teacherId}`}
            className="rounded border border-stone-700 bg-stone-800/60 px-2 py-1"
          >
            <div className="flex items-baseline justify-between gap-2">
              <span className="font-medium text-stone-100">{c.name ?? c.teacherId}</span>
              {c.reasons.some((r) => r.code === 'moduleOwner') && (
                <span className="shrink-0 rounded bg-emerald-500/20 px-1 text-[0.6rem] text-emerald-300">
                  {t('substitute.moduleOwner')}
                </span>
              )}
            </div>
            <p className="text-[0.63rem] text-stone-400">
              {c.reasons
                .filter((r) => r.code === 'load')
                .map((r) =>
                  r.value === 0
                    ? t('substitute.loadNone')
                    : t('substitute.load', { count: String(r.value) })
                )}
            </p>
            {dep && (
              <p
                data-testid={`substitute-deputat-${c.teacherId}`}
                className={`text-[0.63rem] ${wouldExceed ? 'text-amber-300' : 'text-stone-400'}`}
              >
                {canProject
                  ? t('substitute.deputatAfter', {
                      before: String(dep.taught),
                      after: String(after),
                      contract: String(dep.contract),
                    })
                  : t('substitute.deputat', {
                      taught: String(dep.taught ?? '?'),
                      contract: String(dep.contract ?? '?'),
                    })}
                {wouldExceed ? ` · ${t('substitute.wouldExceed')}` : ''}
              </p>
            )}
          </div>
        );
      })}
      {/* The caveat travels with the answer — the same one the assistant states aloud. */}
      <p className="text-[0.6rem] text-stone-500">{t('substitute.competence')}</p>
      {/* ⚠️ Says plainly that this names people and moves nothing, so nobody waits for a change. */}
      <p className="text-[0.6rem] text-stone-500">{t('substitute.readOnly')}</p>
    </div>
  );
}
