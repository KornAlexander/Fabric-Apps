...
import { conflictKey, conflictReason, verdictFor, type MoveVerdict } from '@/planner/moveCheck';
...
import { PlannerWakingNotice } from '@/components/PlannerWakingNotice';
...
export function CalendarPanel({
   ...
   * generated and have no such gaps; only the university's real Untis export does, and a required prop would
  ...
  siteKnown?: boolean;
  /**
   * Did the health check that decides `siteKnown` come back as a genuine failure, rather than the
   * container merely still being cold? See `backendHealthFailed()` in `src/api/scheduler.ts`.
   *
   * ⚠️ Only meaningful while `siteKnown` is false. Ignored once it is true, because by then the
   * outcome is either a served site or a mismatch, and both already have their own honest notice.
   */
  wakeFailed?: boolean;
   ...
   */
  onShowRoute?: (points: [number, number][], people?: number | null) => void;
  /**
   * A transfer that needs the bus was opened: drive the shuttle along it.
   *
   * Raised only for \mode === 'transit'\. The shell owns the 3D scene, so the panel says WHAT
   * happened and never how fast the vehicle should move.
   */
  onRideBus?: (walk: Walk) => void;
  /** Compression of that replay, passed through to WalkList so the factor is shown beside it. */
  ...
  /**
   * Drags the planner has decided to keep, not yet proposed.
   *
   * ⚠️ A QUEUE, BECAUSE MOVES INTERACT. Real replanning is several drags that only make sense
   * together, and two of them can each be legal alone and still collide with each other — same
   * room, same hour. So the batch is re-checked as a SET on every addition, and again on the
   * server before it becomes a proposal.
   */
  const [queued, setQueued] = useState<PendingMove[]>([]);
  ...
  /**
   * How many people the open walking transfer puts on the map.
   *
   * Held here rather than polled off the scene: the count is decided once, when the row is opened,
   * and a per-frame read of the 3D layer would drive a React update behind a panel that is usually
   * shut. Null means the timetable states no size for that cohort — see the badge in `WalkList`.
   */
  const [walkCrowd, setWalkCrowd] = useState<number | null>(null);
  ...
  const replan = useCallback(
    async (day: string) => {
      ...
      if (!subject || replanning) return;
      setReplanning(day);
      setError(null);
      try {
        // Zwei Bereiche, dieselbe Frage: eine Lehrperson fällt aus, oder ein Raum schließt.
        const { affected, proposal } =
          scope === 'room'
            ? await replanWithoutRoom(subject, day)
            : await replanWithout(subject, day);
        const hit = affected.sessions?.length ?? 0;

        if (!hit) {
          // Nothing on that day is not a failure and must not read as one.
          setError(t(scope === 'room' ? 'replan.nothingOnRoom' : 'replan.nothingOn', { day }));
          return;
        }
        if (!proposal) {
          ...
          setError(t(scope === 'room' ? 'replan.noOptionRoom' : 'replan.noOption', { day, n: hit }));
          return;
        }
    ...
    [data, replanning, onProposal, t, scope]
  );
    ...
    try {
      ...
      if (draft.error) {
        ...
        const rows = (diff?.changes ?? []).map((c) => ({
          ...
          // ⚠️ `hand` vs `solver` is not cosmetic. This string is what a reader outside the app has
          // to reconstruct HOW a change was decided, and recording a planner's own drag as
          // `solver;…` would attribute their judgement to a machine — with a `cost` and an
          // `optimalityProven` that no solver ever computed.
          verdict: diff?.handMade
            ? `hand;option=${option};sessionsMoved=${diff?.sessionsMoved ?? ''}`
            : `solver;option=${option};cost=${diff?.cost ?? ''};optimalityProven=${diff?.optimalityProven ?? ''}`,
          conflictCount: 0,
        }));
        setPendingRows(rows);
  ...
  /**
   * Keep the current drag, after proving the WHOLE batch still holds.
   *
   * ⚠️ THE INDIVIDUAL VERDICT IS NOT ENOUGH. `check` judges one move against the published plan,
   * so two queued drags can each be green and still put two sessions in one room at one hour.
   * Nothing in the per-drag verdict can see that, so the set is re-checked here — and the server
   * checks it again before minting a proposal, because this answer can go stale too.
   */
  const enqueue = useCallback(async () => {
    if (!move || queueBusy) return;
    setQueueBusy(true);
    setQueueError(null);
    try {
      const batch = [...queued.filter((q) => q.sessionId !== move.sessionId), move];
      baseline.current ??= await checkMoves([]);
      const after = await checkMoves(
        batch.map((m) => ({ sessionId: m.sessionId, slotId: m.toSlotId, roomId: m.roomId }))
      );
      const known = new Set(baseline.current.map(conflictKey));
      const caused = after.filter((c) => !known.has(conflictKey(c)));
      if (caused.length) {
        // Named, not just refused: the planner has to know WHICH pair of their own drags fight.
        const reason = conflictReason(caused[0]);
        setQueueError(t(reason.key, reason.values as Record<string, string | number>));
        return;
      }
      setQueued(batch);
      setMove(null);
      ...
      const failure = requestError(err);
      setQueueError(t(failure.key, failure.values));
    } finally {
      setQueueBusy(false);
    }
  }, [move, queued, queueBusy, t]);

  /**
   * Hand the queued drags to the confirm gate.
   *
   * Registers them as a proposal and raises the id, exactly as the assistant does with a solver
   * variant — so the preview bar, the option list, "übernehmen" and the draft all work unchanged.
   * Nothing is written here; the human still confirms.
   */
  const proposeQueued = useCallback(async () => {
    if (!queued.length || queueBusy) return;
    setQueueBusy(true);
    setQueueError(null);
    try {
      const result = await proposeHandMoves(
        // Same `?? ''` as the other site-scoped calls here: an empty string lets the server resolve
        // its own default, which is correct for a single-site deployment.
        aoiSchedulerSite ?? '',
        queued.map((m) => ({ sessionId: m.sessionId, slotId: m.toSlotId, roomId: m.roomId }))
      );
      if (result.error) {
        // ⚠️ A SERVER REFUSAL IS SHOWN, NEVER SWALLOWED. It means the plan moved under the planner
        // since they dragged — which is exactly the case this endpoint exists to catch.
        const first = result.caused?.[0] ?? result.undecidable?.[0];
        const reason = first ? conflictReason(first) : null;
        setQueueError(
          reason
            ? t(reason.key, reason.values as Record<string, string | number>)
            : t(`move.refused.${result.error}`)
        );
        return;
      }
      setQueued([]);
  ...
  }, [queued, queueBusy, aoiSchedulerSite, onProposal, t]);
  ...
  /*
   * ⚠️ `!siteKnown` DOES NOT EARLY-RETURN HERE, UNLIKE `mismatch` ABOVE. The detail readout
   * (staffing, quality) is LOCAL data — it does not depend on this backend at all, and an existing
   * gate (`german.spec.ts`: "the guard does not fail open while the backend site is still
   * unknown") already proves it must stay reachable while `/api/health` is hanging. Only the
   * WEEK genuinely comes from this backend, so the waking notice takes the week's place inside the
   * `tab === 'week'` body below, rather than hiding the tabs that lead to the detail readout.
   */
    ...
    >
      ...
      */}
      ...
      <header className="flex shrink-0 flex-wrap items-center gap-3 border-b border-stone-700 px-4 py-2">
        ...
        */}
        ...
        {/*
          By the time this line renders, `!siteKnown` has already returned its own banner above,
          so this is purely the in-grid "fetching the week" indicator — see `PlannerWakingNotice`
          for the cold-start case this used to also (silently, unhelpfully) cover.
        */}
        {loading && <span className="text-xs text-stone-500">{t('calendar.loading')}</span>}
        ...
        {/*
          ...
          Lecturers OR ROOMS. A cohort cannot "become unavailable" in a way the solver models, but a
          room very much can: renovation, water damage, a dead projector, a room let to somebody
          else. That is an ordinary planning event, and it needs no person at all.

          ⚠️ THAT IS WHY THE ROOM DOOR EXISTS, and it is not a workaround. Where the lecturer
          attribution is invented (Garching) the tool layer refuses the person question, which used
          to leave the site with NO route to the solver at all — the one site with a real timetable
          could not show the one thing the product is for. The room, its capacity and its bookings
          are surveyed and real there, so every figure in that answer is one we may state.
          See docs/plan-umplanen-ohne-lehrperson.md.
        */}
        {(scope === 'teacher' || scope === 'room') && data?.subject?.id && !mismatch && (
          <div className="flex items-center gap-1" data-testid="replan-bar">
            <span className="text-[0.65rem] uppercase tracking-wider text-stone-500">
              {t(scope === 'room' ? 'replan.labelRoom' : 'replan.label')}
            </span>
            {(data.days ?? []).map((day) => (
              <button
                key={day}
                ...
                title={t(scope === 'room' ? 'replan.hintRoom' : 'replan.hint', { day })}
      ...
      */}
      ...
      {/*
        ...
        nothing, or this breaks these named things.

        A clean drag can now be KEPT — "vormerken" adds it to a batch, and the batch is registered
        as a proposal that goes through the same confirm gate a solver variant does. What that
        button never does is write: it stages. The plan changes only when a human confirms the
        preview, and only into a draft. A dirty drag still cannot be kept at all, which is why the
        button disappears rather than greying out with an explanation nobody reads.
      */}
      {move && (
        <div
        ...
        >
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            {/* Only for a move that breaks nothing — see the decision above the block. */}
            {verdict?.legal && !checking && !checkFailed && (
              <button
                type="button"
                data-testid="calendar-move-keep"
                disabled={queueBusy}
      ...
      {/*
        The batch a planner has decided to keep. It is deliberately a separate bar from the drag
        verdict: one is a question about a move, this is a list of decisions. Nothing here has been
        written — the primary button hands the list to the SAME confirm gate a solver variant goes
        through, which is what keeps "die Maschine rechnet, die Planung verantwortet" true even
        when the machine did not do the moving.
      */}
      {queued.length > 0 && (
        <div
          data-testid="calendar-queue"
          className="mx-4 mt-2 rounded-lg border border-sky-500/50 bg-sky-500/10 px-3 py-2 text-xs text-sky-100"
        >
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            <span className="font-semibold">{t('move.queued', { count: String(queued.length) })}</span>
            <button
              type="button"
              data-testid="calendar-queue-propose"
              disabled={queueBusy}
              onClick={() => void proposeQueued()}
              className="ml-auto rounded bg-sky-600 px-2 py-0.5 font-medium text-white hover:bg-sky-500 disabled:opacity-50"
            >
              {queueBusy ? t('move.queueBusy') : t('move.queuePropose')}
            </button>
            <button
              type="button"
              data-testid="calendar-queue-clear"
              onClick={() => {
                setQueued([]);
                setQueueError(null);
              }}
              className="rounded px-2 py-0.5 text-sky-200 underline-offset-2 hover:underline"
            >
              {t('move.discard')}
            </button>
          </div>
          ...
          <ul className="mt-1 space-y-0.5">
            {queued.map((q) => (
              <li key={q.sessionId} className="flex items-baseline gap-2 opacity-90">
                <span className="truncate">{q.course}</span>
                <span className="opacity-70">
                  {t('move.target', { from: slotLabel(q.fromSlotId), to: slotLabel(q.toSlotId) })}
                </span>
                <button
                  type="button"
                  data-testid={`calendar-queue-remove-${q.sessionId}`}
                  onClick={() => setQueued((p) => p.filter((x) => x.sessionId !== q.sessionId))}
                  className="ml-auto shrink-0 rounded px-1 text-sky-200 underline-offset-2 hover:underline"
                >
                  {t('move.remove')}
                </button>
              </li>
          ...
          {queueError && (
            <p data-testid="calendar-queue-error" className="mt-1 rounded bg-red-500/20 px-2 py-1 text-red-100">
              {queueError}
            </p>
          )}
          <p className="mt-1 opacity-70">{t('move.queueHint')}</p>
        </div>
      )}
      ...
      {diff && !applied && (
        ...
        >
          ...
          <span className="text-[0.7rem] text-amber-200/80">
            {diff.handMade
              ? t('proposal.handMade')
              : diff.optimalityProven
                ? t('proposal.optimal')
                : t('proposal.bestInTime')}
          </span>
      ...
      */}
      <div ref={bodyRef} className="flex min-h-0 flex-1 flex-col">
        ...
        {!siteKnown ? (
          /*
           * ⚠️ THIS is the "look busy, not broken" fix. Everything above (tabs, the detail
           * readout, subject controls) already worked without the backend; only the table itself
           * has nothing to show until `/api/health` answers, and an empty grid here used to be
           * the entire visible symptom of a cold container.
           */
          <PlannerWakingNotice failed={wakeFailed} />
        ) : (
            ...
            */
            ...
            <WalkList
              ...
              onToggle={(walk) => {
                ...
                setOpenWalk(id);
                /*
                 * How many people actually make this transfer.
                 *
                 * The cohort sitting in the ORIGIN session is the one that gets up and leaves, so
                 * the count comes from the entry the walk starts at. `attendance` is the timetable's
                 * `expectedAttendance` — the size of the cohort group — and it is null on a real
                 * Untis export, which publishes classes without their sizes. Null reaches the scene
                 * unchanged and draws the single walker: the twin says "somebody walks this" when
                 * nobody has said how many, and "twenty-eight people walk this" only when the plan
                 * does.
                 */
                const origin = data?.entries.find((e) => e.sessionId === walk.from.sessionId);
                const people = walk.mode === 'walk' ? (origin?.attendance ?? null) : null;
                setWalkCrowd(people);
                onShowRoute?.(
                  walkRoutes ? routeLine(walkRoutes, walk.fromBuilding, walk.toBuilding) : [],
                  people
                );
                // ⚠️ ONLY WHEN THE TRANSFER IS ACTUALLY BY BUS. Driving the shuttle for a walk
                // between two rooms in one building would show a vehicle crossing town for a
                // journey nobody makes, which is the road being right and the answer being wrong.
                if (walk.mode === 'transit') onRideBus?.(walk);
              }}
            />
          )}
        </div>
