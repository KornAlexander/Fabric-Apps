...
import { createEngineSound, type EngineSound } from '@/twin3d/engineSound';
...
import { backendSite, schedulerConfigured, planFacts, backendHealthFailed } from '@/api/scheduler';
import {
  activeAoi,
  activeAoiId,
  activeBuildingId,
...
/**
 * Wall-clock seconds one bus transfer takes to replay in the twin.
 *
 * ⚠️ TEN, BECAUSE THAT IS ROUGHLY HOW LONG SOMEBODY WILL WATCH A THING THEY CLICKED. The road it
 * covers is 3.0 km and takes 312 s in reality at OTH, so this is about 31x, and LMU's leg works
 * out at 27x. The scale is rendered next to the row that starts it (`busRide`), which is the only
 * reason a compression this large is acceptable here at all: see the header of
 * `src/twin3d/shuttle.ts`, where an unlabelled fixed-duration crossing was removed for implying a
 * 900 km/h bus.
 *
 * ⚠️ Lowering it makes the factor WORSE, not better. Five seconds would be 62x. If this ever needs
 * to be faster, the honest move is to shorten what is watched, not to speed up the vehicle.
 */
const BUS_RIDE_SECONDS = 10;

/**
 * ⚠️ THE CHEAT. Type it anywhere outside a text field and the bus becomes a red sports car.
 *
 * Nothing in the interface offers this, and nothing should: the demo a customer sees is the bus,
 * on the real road, at the compression the badge states. The car is a joke for whoever knows the
 * word, and it is deliberately built so the joke cannot leak into an answer. It changes the mesh,
 * the paint, the replay duration and the noise. It does not change the road, the measured drive
 * time, or the factor reported beside the transfer, which stays honest because it is computed
 * from whatever duration is actually used.
 */
const CHEAT_WORD = 'lambo';

/**
 * How long a cheat-car crossing takes.
 *
 * Four seconds over OTH's 3.0 km road is about 78x real time, against the bus's 31x. That is the
 * entire point of the easter egg and it is why it is behind a word: it is the speed the module
 * header of `src/twin3d/shuttle.ts` argues a REAL vehicle must never move at.
 */
const LAMBO_RIDE_SECONDS = 4;
 ...
 *
 * The dataset knows about Saturday (the university's real export has a Saturday slot), but the played week
...
export function TwinShell() {
  ...
  /**
   * Did the health check that decides `siteKnown` come back as a genuine failure?
   *
   * ⚠️ READ, NOT POLLED. This is set from the SAME `backendSite()` resolution that flips
   * `siteKnown` to true, via `backendHealthFailed()` — see its doc in `src/api/scheduler.ts` for
   * why that is a fact about the one call already made rather than a second request. It can only
   * change in the same tick as `siteKnown`, which is what keeps it from becoming a second, possibly
   * disagreeing truth about whether the backend is known yet.
   */
  ...
  useEffect(() => {
    ...
    backendSite().then((site) => {
      ...
      setServingSite(site);
      setSiteKnown(true);
      setWakeFailed(backendHealthFailed());
    });
    return () => {
      cancelled = true;
    };
  ...
  /**
   * The week drawer and the assistant are both waiting on the same cold container.
   *
   * ⚠️ NOT SHOWN DURING A SITE MISMATCH. A mismatch means the backend already answered — with the
   * wrong university's identity, but it answered — so it is not "still waking up" and has its own,
   * more specific notice.
   *
   * ⚠️ `|| wakeFailed` IS LOAD-BEARING, AND ITS ABSENCE WAS A REAL BUG. On a genuine failure
   * `siteKnown` correctly flips to TRUE — we do now know the answer, and the answer is "it did not
   * work" — so a condition of `!siteKnown` alone could never render the failure state it was
   * built for. `planner-waking.spec.ts` caught exactly that: the hanging case passed, the 500 case
   * found no notice at all.
   *
   * ⚠️ The WEEK DRAWER deliberately does NOT get the same treatment. The week is its own fetch
   * against its own endpoint with its own error handling, so once the site is known it can load
   * perfectly well even if `/api/health` did not — and replacing a week that works with a banner
   * saying the planner did not answer would hide real data behind a false claim.
   */
  const waking = planner && !siteMismatch && (!siteKnown || wakeFailed);
   ...
   */
  const [mainView, setMainView] = useState<'campus' | 'analysis' | 'rules'>('campus');

  /*
   * ⚠️ THE CHEAT, AND ITS ONLY STATE. Off until somebody types the word; there is no control for
   * it and it is not persisted, so a reload puts the bus back. That last part is on purpose: an
   * easter egg that survives a refresh is a setting somebody will demo to a customer by accident.
   */
  const [cheatCar, setCheatCar] = useState(false);
  const engine = useRef<EngineSound | null>(null);
  const engineTimer = useRef<number | null>(null);

  // Listen once. `watchForCheat` ignores text fields, so typing "lambo" into the assistant does
  // nothing, which is the difference between an easter egg and a booby trap.
  useEffect(() => watchForCheat(CHEAT_WORD, () => setCheatCar(true)), []);

  // Push the choice into the scene whenever either changes: the handle arrives asynchronously, so
  // typing the word before the twin has loaded must still take effect when it does.
  useEffect(() => {
    handle?.setShuttleVehicle(cheatCar ? 'sportscar' : 'bus');
  }, [handle, cheatCar]);

  // ⚠️ THE AUDIO CONTEXT OUTLIVES REACT UNLESS SOMEBODY CLOSES IT. Navigating away from the twin
  // with an oscillator running leaves a process making noise at a page that no longer exists.
  useEffect(
    () => () => {
        ...
        */}
        <NavRail
          ...
          items={[
            ...
            {
              /*
                The Regelwerk: the rules the solver obeys, and the room capacities it obeys them
                with (PLAN §26.8, §39).

                ⚠️ It belongs on the RAIL rather than inside the week drawer, because it is not a
                fact about a week — it is the standing configuration every week is judged against.
                Burying it under the calendar would make it discoverable only to somebody already
                looking at the plan, and §26.8's whole argument for a panel over a chatbot is that
                a panel makes rules visible WITHOUT BEING ASKED.
              */
              id: 'rules',
              available: planner,
              active: mainView === 'rules',
              onToggle: () => setMainView((v) => (v === 'rules' ? 'campus' : 'rules')),
            },
            {
              id: 'assistant',
        ...
        <main className="relative min-w-0 flex-1">
          ...
          */}
          ...
          {/*
            The Regelwerk — an OVERLAY for the same reason the Auswertung is one: rendering it
            INSTEAD of `<Twin3DView>` would unmount the scene and dispose every geometry, material
            and texture on the campus, then rebuild all of it on the way back.

            ⚠️ Rendered only for a site that HAS a planner. The rules are the solver's rules and
            the room table is the planner's stock; on a twin with no timetable both would be a
            settings screen for a feature that site does not have.
          */}
          {mainView === 'rules' && planner ? (
            <div className="absolute inset-0 z-30 overflow-auto bg-stone-950/95 backdrop-blur">
              <RulesPanel site={aoi.schedulerSite} />
            </div>
          ) : null}
          ...
          */}
          ...
          {/*
            Which way is north — top left, clear of the drone hint (top right) and the HUD (bottom
            centre), and under the Auswertung overlay's `z-30` so it does not float over a screen
            that has no map on it.
          */}
          {handle && <Compass handle={handle} />}
          ...
          */}
          ...
          {planner && (
            <CalendarPanel
              ...
              aoiId={aoiId}
              onShowRoute={(points, people) => handle?.showWalkRoute(points, people)}
              /*
               * ⚠️ TEN SECONDS FOR A FIVE-MINUTE DRIVE, ON PURPOSE AND WITH THE FACTOR ON SCREEN.
               * Opening a transfer that needs the bus sends the shuttle along the real road, but
               * compressed, because the question the row asks is "can this cohort make it" and
               * that cannot be watched at 34 km/h. `src/twin3d/shuttle.ts` explains at length why
               * an UNLABELLED fast bus was removed once already; `busRide` below is what keeps
               * this one honest, and the two must be changed together.
               *
               * The direction is taken from the transfer, so a cohort travelling back to
               * Seybothstraße does not watch a bus drive away from them.
               */
              onRideBus={(walk) => {
                const leg = handle?.shuttleLeg();
                /*
                 * ⚠️ `campusId` IS OPTIONAL AND OFTEN THE ONLY THING PRESENT. The university's own
                 * export gives a campus for every Prüfeninger-Straße booking and a building for
                 * none of them, so keying the direction off the building id would silently drive
                 * every OTH transfer forwards. When the origin campus is unknown, forwards is the
                 * honest default: it is the leg as measured, not a guess about the traveller.
                 */
                const origin = walk.from.campusId ?? null;
                const seconds = cheatCar ? LAMBO_RIDE_SECONDS : BUS_RIDE_SECONDS;
                handle?.playShuttleJourney(
                  seconds,
                  Boolean(leg && origin && origin === leg.to)
                );

                if (!cheatCar) return;
                /*
                 * ⚠️ SOUND ONLY UNDER THE CHEAT, AND ONLY WHILE THE CAR IS ACTUALLY MOVING. An app
                 * that makes noise on an ordinary click is an app people mute once and never
                 * unmute. It is also stopped by a timer rather than left to run: a fire-and-forget
                 * engine that outlives its journey is the classic way an easter egg becomes a
                 * support ticket.
                 */
                engine.current = engine.current ?? createEngineSound();
                engine.current.start();
                engine.current.setLoad(1);
                if (engineTimer.current !== null) window.clearTimeout(engineTimer.current);
              ...
              busRide={
                handle?.shuttleLeg()
                  ? {
                      realSeconds: Math.round(handle.shuttleLeg()!.driveSeconds),
                      // ⚠️ THE BADGE FOLLOWS THE CHEAT. Leaving it at 10 s while the car crosses
                      // in 4 would print a factor that is simply wrong, and the whole argument for
                      // allowing a compressed crossing at all is that the number beside it is true.
                      shownSeconds: cheatCar ? LAMBO_RIDE_SECONDS : BUS_RIDE_SECONDS,
                    }
                  : null
              }
            /*
                ...
                */}
                ...
                >
                  {!planner ? (
                     ...
                     *
                     * ⚠️ TWO WORDINGS, BECAUSE ONE OF THEM CONTRADICTED THE PANEL BELOW IT. The
                     * single message promised "Gebäude, Räume und Analysen sind vorhanden" — true
                     * of every twin that had ever reached this branch, all of which came from
                     * Campus-Insights with room data. `uni-regensburg` is the first site with a
                     * scene and no interiors, so it claimed rooms in one box while `lens.noRooms`
                     * denied them in the next, on the same screen. Keyed on `aoi.rooms` because
                     * `lensesFor` returns nothing without room data, so that one field settles
                    ...
                    >
                      {t(aoi.rooms ? 'site.noPlanner' : 'site.noPlannerSceneOnly')}
                    </p>
                  ) : siteMismatch ? (
                    /*
                    ...
                    /*
                     * ⚠️ SAME REASON AS THE MISMATCH BRANCH ABOVE, DIFFERENT CAUSE. Rendering
                     * `PlannerChat` while the container is still cold used to leave the column
                     * looking like it had simply failed to load — nothing said the assistant was
                     * one health check away from working, so a slow demo network read as a broken
