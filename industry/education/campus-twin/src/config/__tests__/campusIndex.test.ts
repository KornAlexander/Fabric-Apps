...
describe('the national campus index', () => {
  ...
  it('offers a dot for every AOI that is built', () => {
    // ⚠️ ONE EXCEPTION, AND IT IS NOT A TWIN THAT WAS FORGOTTEN. `campus-demo`
    // (Beispiel-Universität) is the generic site: an invented institution on the reference
    // build's ground. This index is a registry of REAL German universities with real student
    // numbers, and the demo would land a fictional dot on exactly TU Berlin's coordinates —
    // a fake university sitting on top of a real one. It stays reachable through the site
    // dropdown and through `?aoi=campus-demo`, so it is not the dead end this guard is for.
    const NOT_ON_THE_MAP = new Set(['campus-demo']);
    const reachable = new Set(dots.map((d) => d.aoi).filter(Boolean));
    const unreachable = Object.keys(AOIS).filter(
      (id) => !reachable.has(id) && !NOT_ON_THE_MAP.has(id)
    );
