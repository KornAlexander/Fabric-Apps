...
import campusDemo from '@config/aoi/campus-demo.json';
...
import tuBerlin from '@config/aoi/tu-berlin.json';
import uniRegensburg from '@config/aoi/uni-regensburg.json';
...
export interface AoiConfig {
  id: string;
  /**
   * Serve this AOI's generated terrain from ANOTHER AOI's folder.
   *
   * ⚠️ ONLY for a site that is deliberately the same ground under a different name. The generic
   * `campus-demo` is TU Berlin's box presented as an anonymous university, so duplicating
   * `public/terrain/tu-berlin/` would put the same 48 MB into the deployed bundle twice for
   * byte-identical pixels. Everything ELSE about the two sites differs — names, campuses,
   * faculties, timetable — which is exactly why this is one optional field rather than a general
   * "clone a site" mechanism.
   *
   * ⚠️ It does NOT redirect the scheduler dataset. That is `schedulerSite`, and the demo has its
   * own; sharing the ground is not sharing the week.
   */
  assetsFrom?: string;
  site: {
    name: Record<string, string>;
 ...
 *
 * ⚠️ EXACTLY ONE OF THESE HAS NO PLANNER BACKEND, AND THAT IS THE POINT OF `schedulerSite`.
 * Garching and Tübingen came from Campus-Insights, which is a campus twin with no solver behind it;
 * FAU Erlangen, Köln, Aachen and Münster were built later for the same reason — they are the
 * largest German universities this project can reach, and none of them publishes a timetable. All
 * six were later given a GENERATED one, so the only site left with a scene and no week to replan is
 * `uni-regensburg`. Anything that needs the backend keys off `schedulerSite` being present rather
 * than assuming every site has one — see `TwinShell`.
 *
 * ⚠️ THAT LAST ONE IS LOAD-BEARING, NOT A GAP. `rail.spec.ts` asserts that a twin with no timetable
 * offers no week, no walks and no changes; with every site carrying a planner there is no subject
 * left to assert it against, and the guard silently stops guarding. It was already in that state
 * once — see PLAN §76.
 *
...
const ALL_AOIS: Record<string, AoiConfig> = {
  ...
  'uni-regensburg': uniRegensburg as unknown as AoiConfig,
  ...
  // ⚠️ The only AOI in this registry that does not work in UTM zone 32 — Berlin surveys in
  // EPSG:25833. `worldExtent` reads the zone from `workingCrs` and carries it on the extent for
  // exactly this entry's sake; anything that reintroduces a zone-32 constant breaks this site
  // alone, and breaks it by a few metres rather than visibly.
  'tu-berlin': tuBerlin as unknown as AoiConfig,
  // ⚠️ THE ONLY AOI HERE THAT IS NOT A REAL INSTITUTION, and the only one that serves another
  // site's terrain (`assetsFrom: 'tu-berlin'`). It exists so the product can be shown to a
  // university that is not one of the nine, without another university's name on the screen.
  'campus-demo': campusDemo as unknown as AoiConfig,
};
