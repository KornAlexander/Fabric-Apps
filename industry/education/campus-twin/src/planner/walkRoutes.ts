 ...
 *
 * `campus` means at least one room resolved only as far as its site — the university's export numbers the
...
export interface PlacedSession {
   ...
   *
   * the university's own export gives us this for every Prüfeninger-Straße booking and a building for none of
...
export function weekWalks(
  ...
  for (const [dayIndex, list] of [...byDay.entries()].sort((a, b) => a[0] - b[0])) {
    ...
    for (let i = 0; i < list.length - 1; i += 1) {
      ...
      // ⚠️ TWO SESSIONS THAT OVERLAP ARE A CLASH, NOT A WALK. the university's own export is a weekly grid
