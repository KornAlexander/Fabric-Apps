...
describe('fly controls', () => {
  ...
  /**
   * ⚠️ THE HEADING READ BACKWARDS FOR AS LONG AS THIS INSTRUMENT HAS EXISTED, and the test above
   * is why nobody noticed: north is 0 whichever way you count, so pinning only north blesses a
   * mirrored compass. Flying due east printed **270**.
   *
   * These are facts about the scene's world (`scene.ts` puts east on +x and south on +z), not
   * about the formula, so they are the assertion that has teeth.
   */
  it.each([
    ['north', new THREE.Vector3(0, 0, -1), 0],
    ['east', new THREE.Vector3(1, 0, 0), 90],
    ['south', new THREE.Vector3(0, 0, 1), 180],
    ['west', new THREE.Vector3(-1, 0, 0), 270],
  ])('reads a heading of %s clockwise from north', (_name, direction, expected) => {
    camera.lookAt(camera.position.clone().add(direction));
    expect(fly.telemetry().headingDeg).toBeCloseTo(expected, 0);
  });

  describe('facing north', () => {
    it('swings the gimbal round to north without moving the camera', () => {
      camera.lookAt(camera.position.clone().add(new THREE.Vector3(1, 0, 0))); // due east
      const stood = camera.position.clone();
      // ⚠️ Engaged explicitly, NOT with `w`: the gimbal is the drone's to turn so it has to own the
      // camera first, but W also flies it — the first version of this test measured 200 m of
      // "drift" that was the throttle doing exactly what it should.
      fly.setEngaged(true);
      fly.faceNorth();
      tick(1);

      expect(fly.telemetry().headingDeg).toBeCloseTo(0, 0);
      // A compass turns the view. It does not fly anywhere.
      expect(camera.position.distanceTo(stood)).toBeLessThan(1e-6);
    });

    it('takes the short way round, however many turns the camera has done', () => {
      camera.lookAt(camera.position.clone().add(new THREE.Vector3(-0.2, 0, -1))); // just west of north
      fly.setEngaged(true);
      // Two and a bit full turns, the way holding R does. `targetYaw` accumulates, so this is the
      // case where easing towards a bare zero would unwind two revolutions on screen.
      press('r');
      tick((2 * Math.PI * 2) / SPIN_PER_SECOND);
      release('r');

      const before = new THREE.Vector3();
      camera.getWorldDirection(before);
      fly.faceNorth();
      // A single frame's worth of the look stabiliser. If the turn were unwinding whole
      // revolutions this could not be within a few degrees of north yet.
      tick(0.5);
      expect(fly.telemetry().headingDeg % 360).toBeCloseTo(0, 0);
      // And it really did turn the short way: less than half a circle of total swing.
      const after = new THREE.Vector3();
      camera.getWorldDirection(after);
      expect(after.angleTo(before)).toBeLessThan(Math.PI);
    });
  });
