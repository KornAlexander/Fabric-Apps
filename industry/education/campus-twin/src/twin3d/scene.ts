...
import { TERRAIN_BASE } from '@/config/terrainBase';
...
export interface Campus3DHandle {
   ...
   *
   * `people` is the cohort making the transfer, from the timetable's `expectedAttendance`. Given a
   * number, that many figures walk it; given null — which is what a real Untis export yields,
   * because Untis publishes classes and not their sizes — the single walker is drawn instead.
   */
  showWalkRoute(points: [number, number][], people?: number | null): void;
   ...
   *
   * Reports the leader when a crowd is walking, so the timing checks are the same either way.
   */
  walker(): { x: number; z: number; progress: number; seconds: number } | null;
  /**
   * The crowd on the drawn route, or null when it is walked by the single figure.
   *
   * `people` is what the timetable stated; `drawn` is how many are on screen, which is smaller
   * only for a cohort past the layer's cap.
   */
  walkCrowd(): { people: number; drawn: number } | null;
  ...
  /**
   * Which way the view points, in radians clockwise from north.
   *
   * Answers for BOTH camera modes — the orbit azimuth while the map has the camera, the gimbal
   * bearing while the drone does. A compass that freezes the moment somebody presses W is worse
   * than no compass, because it is then confidently wrong rather than absent.
   */
  headingRad(): number;
  /**
   * Turn the view back to north. Where the camera is, how far away and how steeply it looks are
   * all left alone — that is the whole promise a compass makes, and reusing the camera flight
   * would have re-framed the shot as well.
   */
  faceNorth(): void;
  ...
  /**
   * Watch one crossing, compressed into `seconds` of wall clock.
   *
   * ⚠️ THIS ALSO MAKES THE BUS VISIBLE, and it has to. The ambient shuttle is only shown while the
   * week is playing (`setWeekTime`), so asking to watch a journey from a paused week would
   * otherwise start a replay of something nobody can see, and the button would look dead.
   */
  playShuttleJourney(seconds: number, reverse?: boolean): void;
  /**
   * Swap the vehicle on the road. ⚠️ `sportscar` is an easter egg behind a typed cheat word.
   *
   * Route, timing and the reported compression are unchanged: only the mesh and the paint. A cheat
   * that also moved the road would be a second code path able to tell a different story.
   */
  setShuttleVehicle(kind: 'bus' | 'sportscar'): void;
  shuttleVehicle(): 'bus' | 'sportscar';
  /**
   * The replay in progress, or null. Carries the compression factor so the panel can show it.
   */
  shuttleJourney(): {
    progress: number;
    factor: number;
...
export async function initCampus3D(
  ...
  const ext: WorldExtent = worldExtent(aoi);

  /**
   * Which AOI's folder the GROUND comes from.
   *
   * ⚠️ THIS IS THE SHARED HALF, AND THE SPLIT IS THE WHOLE POINT. Terrain, drape, buildings,
   * trees, water and the shuttle route are properties of a place, so a site declaring
   * `assetsFrom` serves another site's copies and the deployed bundle carries them once.
   * Rooms, occupancy, flows, plan quality and staffing are properties of a WEEK — they are
   * derived from this site's own timetable and must stay on `aoi.id`, or the generic demo would
   * show the reference build's week under its own name.
   *
   * For every AOI except `campus-demo` this is just `aoi.id`.
   */
  const assetAoi = aoi.assetsFrom ?? aoi.id;
  ...
  try {
    assets = await loadTerrain(assetAoi, TERRAIN_BASE, options.onProgress);
  } catch (error) {
  ...
  if (assets && terrain) {
    ...
    const material = createTerrainMaterial({
      ...
      shellPhoto: Boolean(assets.shellDrapeTexture),
    });
    ...
    if (shell && shellTexture) {
      ...
      const shellMaterial = createShellMaterial({
        ...
        // …and where the shell has been photographed, the ramp is not used at all.
        shellDrapeTexture: assets.shellDrapeTexture,
      });
  ...
  if (hasTerrain) {
    try {
      buildings = await loadBuildings(assetAoi, TERRAIN_BASE, options.onProgress);
  ...
  if (hasTerrain) {
    try {
      vegetation = await loadVegetation(assetAoi, TERRAIN_BASE, options.onProgress);
      if (vegetation) {
  ...
  if (hasTerrain && aoi.rooms) {
    try {
      rooms = await loadRooms(aoi.id, TERRAIN_BASE);
      if (rooms) {
  ...
  /**
   * Does this label name a whole SITE rather than a building?
   *
   ...
   * so a nearest-building rule opened one arbitrary building and presented it as the campus.
   *
   * ⚠️ CROSS-REFERENCING `campuses[]` WAS NOT ENOUGH, AND MOSTLY DID NOTHING. Campus ids and focus
   * place ids are separate namespaces that only sometimes coincide: on six of the nine AOIs every
   * declared campus id matches no focus place at all, so the set was empty and the rule inert. It
   * happened to work at OTH and LMU because `pruefening` and `klinikum` are spelled the same in
   * both lists.
   *
   * Garching is where that bit. Its label "Campus Garching" is named in no `campuses[]` block, so
   * clicking it opened the Galileo Conference Center — 44 m away, and not the campus — and
   * presented it as the site. `kind: 'campus'` is what `PlaceNote` has always keyed its "this is a
   * campus outline, pick a building" message on, so a place now says what it is instead of being
   * recognised by a list somewhere else. The `campuses[]` lookup is kept as well: it is what the
   * sites that already work rely on.
   */
  const campusPlaceIds = new Set((aoi.campuses ?? []).map((campus) => campus.id));
  const namesASite = (place: AoiFocusPlace) =>
    place.kind === 'campus' || campusPlaceIds.has(place.id);
  ...
  const buildingNearPlace = (place: AoiFocusPlace): string | null => {
    if (!rooms || namesASite(place)) return null;
   ...
   *
   * Kept apart from `frame()` because it is a different move: the target, the distance and the
   * tilt all stay exactly where they are and only the bearing changes. Running it through the
   * camera flight would have re-framed the shot, which is not what a compass promises.
   */
  let northing: { from: number; delta: number; startedAt: number } | null = null;
  const NORTHING_MS = 700;
  const northingSpherical = new THREE.Spherical();
  const northingOffset = new THREE.Vector3();
  const northingForward = new THREE.Vector3();

  const headingRad = () => {
    if (drone.engaged) {
      camera.getWorldDirection(northingForward);
      return headingRadFromForward(northingForward.x, northingForward.z);
    }
    northingOffset.copy(camera.position).sub(controls.target);
    northingSpherical.setFromVector3(northingOffset);
    return headingFromAzimuth(northingSpherical.theta);
  };

  const setHeading = (heading: number) => {
    northingOffset.copy(camera.position).sub(controls.target);
    northingSpherical.setFromVector3(northingOffset);
    northingSpherical.theta = azimuthFromHeading(heading);
    northingOffset.setFromSpherical(northingSpherical);
    camera.position.copy(controls.target).add(northingOffset);
    controls.update();
  };

  const faceNorth = () => {
    // The drone's gimbal is its own to turn — it holds the yaw, and writing the camera's
    // quaternion behind its back would be undone on its very next frame.
    if (drone.engaged) {
      drone.faceNorth();
      return;
    }
    ...
    // A flight is also steering; two things easing the same camera towards different places is
    // how a control ends up looking broken rather than busy.
    flightFrom = flightTo = flightTargetFrom = flightTargetTo = null;
    northing = { from, delta: shortestTurnToNorth(from), startedAt: performance.now() };
  };

  const advanceNorthing = () => {
    if (!northing) return;
    const k = Math.min(1, (performance.now() - northing.startedAt) / NORTHING_MS);
    // The same smoothstep the camera flights use, so the two moves feel like one camera.
    setHeading(northing.from + northing.delta * (k * k * (3 - 2 * k)));
  ...
  if (hasTerrain) {
    try {
      flows = await loadFlows(aoi.id, TERRAIN_BASE, groundAt);
      if (flows) {
  ...
  try {
    shuttle = await loadShuttle(assetAoi, TERRAIN_BASE, ext, groundAt);
    if (shuttle) {
  ...
  const tick = () => {
    ...
    //
    // ⚠️ A REQUESTED JOURNEY IS THE EXCEPTION, AND IT IS COMPLETED RATHER THAN ANIMATED. Skipping
    // it outright would honour the preference by making the feature dead: a viewer on reduced
    // motion clicks a transfer, nothing happens, and nothing says why. Handing the replay its
    // whole duration in one frame moves the bus to the far campus without a journey across the
    // screen, which is what "reduced motion" asks for, and the panel still shows the arrival.
    if (shuttle?.visible()) {
      const replay = shuttle.journey();
      if (reduced) {
        if (replay) shuttle.tick(replay.shownSeconds);
      } else {
        shuttle.tick(dt);
      }
    }
    ...
    if (flightFrom && flightTo && flightTargetFrom && flightTargetTo) {
      // A flight re-frames the whole shot, so it outranks a compass turn — and letting both run
      // would have two easings writing the camera position on the same frame.
      northing = null;
  ...
  const handle: Campus3DHandle = {
    ...
    showWalkRoute(points, people = null) {
      if (points.length < 2) {
        walkRoute.clear();
        return;
      }
      walkRoute.show(points, undefined, people);
     ...
     */
    setWeekTime(time) {
      // ⚠️ A REPLAY OUTRANKS THE WEEK CLOCK. Pausing the week while a journey is running would
      // hide the very thing the user asked to watch, and `playShuttleJourney` turned it on.
      if (shuttle?.journey()) return;
    ...
    playShuttleJourney(seconds, reverse = false) {
      if (!shuttle) return;
      shuttle.setVisible(true);
      shuttle.playJourney(seconds, reverse);
    },
    setShuttleVehicle(kind) {
      shuttle?.setVehicle(kind);
    },
    shuttleVehicle() {
      return shuttle?.vehicle() ?? 'bus';
    },
    shuttleJourney() {
      return shuttle?.journey() ?? null;
    },
