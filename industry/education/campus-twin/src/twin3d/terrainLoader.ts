...
export interface TerrainAssets {
  ...
  /**
   * The same orthophoto over the coarse shell, at about a twentieth of the resolution.
   *
   * Optional, and the reason it exists is that the alternative was invented colour: without it the
   * shell is painted by a hypsometric ramp, which on a flat site renders twenty kilometres of city,
   * river, forest and farmland as one pale-green sheet. Showing the SAME ground at two sharpnesses
   * is what makes the boundary between the tiers disappear.
   */
  shellDrape: DrapeMeta | null;
...
export async function loadTerrain(
  ...
  const [terrain, landuse, shell, drape, shellDrape] = await Promise.all([
    ...
    // The horizon's orthophoto. Newer than the other four, so every site that predates it must go
    // on rendering exactly as before rather than failing to load.
    fetchJson<DrapeMeta>(`${root}/shell-drape.json`).catch(() => null),
  ]);
  ...
  // No progress stage of its own: it is well under a megabyte, and a second bar that fills
  // instantly reads as a stutter rather than as information.
  const shellDrapeTexture = shellDrape ? await loadDrape(root, shellDrape) : null;
