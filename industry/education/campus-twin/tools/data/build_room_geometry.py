...
# ⚠️ WHERE THE GROUND IS READ FROM, which is not always where the rooms are written to. A site
# whose AOI declares `assetsFrom` shares another site's heightmap; its rooms are still its own.
# Identical to OUT for every site except the generic demo.
ASSETS: Path = SITE.asset_dir()
...
def world_extent() -> tuple[float, float, float, float]:
    ...
    b = AOI["bbox"]
    corners = [
        wgs84_to_utm(b["west"], b["south"]),
        wgs84_to_utm(b["west"], b["north"]),
        wgs84_to_utm(b["east"], b["north"]),
        wgs84_to_utm(b["east"], b["south"]),
    ]
...
class Terrain:
    ...
    def __init__(self) -> None:
        meta = json.loads((ASSETS / "heightmap.json").read_text(encoding="utf-8"))
        ...
        raw = np.fromfile(ASSETS / "heightmap.u16", dtype="<u2").reshape(self.h, self.w)
...
def main() -> None:
    ...
    global SITE, AOI, BUILDINGS, OSM_ROOMS, PLAN_ROOMS, SYNTH, OUT, ASSETS, STOREY_M
    SITE = load_site(args.site)
    AOI = SITE.aoi()
    BUILDINGS = SITE.buildings_payload() or {"buildings": []}
