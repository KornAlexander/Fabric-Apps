...
def building_footprints(geometry: dict) -> dict[str, int]:
    """Plan-view extent of each building, in square metres, from its own rooms.

    ⚠️ THIS EXISTS BECAUSE TUM WAS THE ONLY SITE THE WALK ROUTER COULD NOT RUN ON.
    `tools/data/build_walk_routes.py` picks one stand-in building per campus with
    `max(on_campus, key=lambda b: (b["footprintM2"], b["buildingId"]))`, and TUM's
    `building.json` was the only one of nine datasets without that field, so the router
    died on a KeyError before it could write anything. That single missing field is why
    Garching had no `walk-routes.json` and why its travel matrix was still straight lines
    long after every other site had been routed.

    ⚠️ IT IS A HULL, NOT A FOOTPRINT, AND THE DIFFERENCE IS REAL. This is the area of the
    convex hull of every room outline in the building, across ALL floors, so it fills
    courtyards and light wells and it takes the widest floor as if it were the ground one.
    It is therefore an OVER-estimate of the true footprint wherever a building is not
    convex. That is acceptable for the one thing the value is used for -- ranking buildings
    by size to choose a campus stand-in -- and it would NOT be acceptable as a capacity or
    a rent figure. Everywhere else in this repository `footprintM2` comes from a surveyed
    LoD2 or OSM outline; here it is derived, and `footprintSource` on every row says so.

    The rooms themselves are real OpenStreetMap indoor geometry, so the hull is measured
    rather than assumed. Being a hull of INTERIOR outlines it also stops at the inside face
    of the walls, which pulls the estimate back down a little.
    """
    scale = geometry["quantisation"]["xzScaleM"]
    raw = (TERRAIN / "rooms.bin").read_bytes()
    verts = struct.unpack(f"<{len(raw) // 2}h", raw)
    ...
    for room in geometry["rooms"]:
        ...
        xs = verts[off * 2: (off + n) * 2: 2]
        zs = verts[off * 2 + 1: (off + n) * 2: 2]
        points[building].extend((x * scale, z * scale) for x, z in zip(xs, zs))

    return {b: round(_hull_area(pts)) for b, pts in points.items() if pts}


def _hull_area(points: list[tuple[float, float]]) -> float:
    """Area of the convex hull of a point set (monotone chain, then the shoelace).

    Written out rather than pulled from scipy because this module has no scientific
    dependency and one field does not justify acquiring one.
    """
...
def main() -> None:
    ...
    footprints = building_footprints(geometry)
    navigatum_names = {}
    ...
    for code in building_ids:
        ...
        buildings.append({
            ...
            "lat": pos.get("lat"),
            "lon": pos.get("lon"),
            "footprintM2": footprints.get(code, 0),
            "footprintSource": "derived: convex hull of the building's own OSM room outlines, "
                               "over all floors. Every other site measures this from a surveyed "
                               "LoD2 or OSM building outline; TUM has no such outline per code, "
                               "so the hull stands in. Used only to rank buildings by size.",
            "provenance": "measured — NavigaTUM name and address, position from OSM room geometry",
        })

    # ── travel time ───────────────────────────────────────────────────────────────────
    #
    # Prefers `travel_routed.json`, real shortest paths on the OpenStreetMap pedestrian
    # network, written by `build_walk_routes.py --site tum --matrix`. Falls back to straight
    # lines, loudly. This mirrors `generate_timetable.py.travel_time_rows()` deliberately:
    # TUM used to be the ONE site that could never reach the routed branch, and having two
    # different answers to "how long is this walk" in one repository is how the two drift.
    #
    # ⚠️ THE TWO-PASS ORDER IS DELIBERATE AND HAS TO BE, exactly as for the generated sites.
    # The router reads `building.json` to know which buildings exist and where they are, and
    # THIS script writes that file, so the first pass bootstraps the buildings with straight
    # lines, the router measures them, and a second pass plans against the measurements.
    #
    #     python tools/data/build_tum_dataset.py
    #     python tools/data/build_walk_routes.py --site tum --matrix
    #     python tools/data/build_tum_dataset.py
    #
    # ⚠️ AND THE FIRST PASS IS NOT COSMETIC. Garching is one campus, so every transfer here is
    # a walk between blocks; a matrix that is optimistic by roughly a tenth is a matrix that
    # quietly says a lecturer can make a change of room they cannot make.
    routed_path = OUT / "travel_routed.json"
    if routed_path.exists():
        travel = json.loads(routed_path.read_text(encoding="utf-8"))
        known = set(building_ids)
        covered = {r["fromBuildingId"] for r in travel} | {r["toBuildingId"] for r in travel}
        missing = known - covered
        if missing:
            # A partial matrix is worse than none: an absent pair reads to the placer as
            # "no travel time at all", which is indistinguishable from "these rooms adjoin".
            raise SystemExit(
                f"{routed_path.name} covers {len(covered)} buildings but the dataset has "
                f"{len(known)}; missing {sorted(missing)}. Re-run "
                f"`build_walk_routes.py --site tum --matrix`."
            )
        walked = sum(1 for r in travel if r.get("mode") == "walk")
        print(f"travel      routed ({len(travel)} pairs, {walked} on foot) "
              f"from {routed_path.name}")
    else:
        print(f"travel      ⚠️  STRAIGHT-LINE fallback: {routed_path.name} is missing. "
              f"Run `python tools/data/build_walk_routes.py --site tum --matrix` first.")
        # ⚠️ STRAIGHT LINES, AND SAID SO. These are real distances between real building
        # centroids, but a straight line is not a walk. Every row is marked `straight` so a
        # later routed build can replace it without anyone having to guess which is which.
        #
        # ⚠️ `straight` IS NOT A `TransferMode`. src/planner/walkRoutes.ts declares
        # `'none' | 'walk' | 'transit'`, so a row left in this state carries a mode the front
        # end does not know and will not draw. That is the intended pressure: the fallback is
        # supposed to be visibly unfinished, not a quiet second-class answer.
        WALK_M_PER_MIN = 80.0
        travel = []
