...
import sites
from sites import add_site_argument
...
#
# ⚠️ THE DEFAULT, NOT THE LAW — an AOI may raise it in `walkRouting.maxSameCampusDetour`, and
# exactly one has. The number was calibrated on OTH and LMU, whose campuses are open sites you can
# cut across, and it does not survive contact with a baroque town: Erlangen's Innenstadt campus is
# arranged around the Schlossgarten and the Botanischer Garten, both WALLED and gated, so the 206 m
# from the Audimax to the Nikolaus-Fiebiger-Zentrum is a 1036 m walk around the wall. That is not a
# broken access point, it is the reason a timetable should care about the walk at all.
#
# ⚠️ RAISE IT ONLY AFTER READING THE DIAGNOSTIC THE GATE NOW PRINTS. The signature of a real
# modelling artefact is a handful of buildings appearing in most of the bad pairs with a large
# `approachM`; the signature of real geography is a smooth tail (Aachen's worst pair is 3.13x and
# its second-worst 2.94x) and healthy access points on every building involved.
...
def main() -> None:
    ...
    # ⚠️ THE SITE REGISTRY, NOT A HAND-WRITTEN PAIR. This read `choices=["oth", "lmu"]`, which was
    # true when there were two universities and became a silent ceiling when there were seven: the
    # five twins brought up to a full scheduler could generate a timetable, rooms, occupancy,
    # staffing and plan quality, and then could not have their walk routes built — with an
    # argparse error naming the two sites that happen to be listed here rather than the ones the
    # registry knows. `add_site_argument` is the registry's own accessor and is what every other
    # script in this folder already uses; that is the whole point of `sites.py` existing.
    ...
    names = {b["buildingId"]: (b.get("name") or "(unnamed)") for b in buildings}
    impossible: list[str] = []
    worst_ratio, worst_pair = 0.0, ""
    # ⚠️ THE GATE'S OWN ADVICE USED TO BE UNACTIONABLE. It said "check whether the same few
    # buildings appear in every bad pair" and then printed exactly ONE pair, by two-letter
    # placeholder id, with no names and no distances — so the first thing anyone had to do was
    # re-derive the list the gate had just computed and thrown away. Keeping the worst offenders
    # costs a list of floats and turns the failure into a diagnosis.
    ratios_by_pair: list[tuple[float, str, float, float]] = []
    for key, route in routes.items():
        ...
        if direct > 25 and route["sameCampus"]:
            ...
            ratios_by_pair.append((ratio, key, direct, float(route["distanceM"])))
    ...
    max_detour = float(aoi.get("walkRouting", {}).get("maxSameCampusDetour", MAX_SAME_CAMPUS_DETOUR))
    if max_detour != MAX_SAME_CAMPUS_DETOUR:
        print(f"  (this AOI raises the limit to {max_detour}x — see walkRouting in its config)")
    if worst_ratio > max_detour:
        offenders = Counter()
        for ratio, key, _, _ in ratios_by_pair:
            if ratio > max_detour:
                left, right = key.split("|")
                offenders[left] += 1
                offenders[right] += 1
        worst_lines = [
            f"{key:>12}  {ratio:5.2f}x  direct {direct:6.0f} m  walked {walked:6.0f} m  "
            f"{names[key.split('|')[0]][:34]} -> {names[key.split('|')[1]][:34]}"
            for ratio, key, direct, walked in sorted(ratios_by_pair, reverse=True)[:10]
        ]
        repeat_lines = [
            f"{bid:>4}  in {n:3} bad pairs  {names[bid][:60]}"
            for bid, n in offenders.most_common(8)
        ]
        raise SystemExit(
            f"REFUSING TO WRITE: {worst_pair} detours {worst_ratio:.1f}x the direct line on one "
            f"campus, over the {max_detour}x limit. That is normally an access-point "
            f"problem (a building attached to a stub of the network), not real geography.\n\n"
            f"worst pairs:\n  " + "\n  ".join(worst_lines) + "\n\n"
            f"buildings appearing in the {sum(1 for r, *_ in ratios_by_pair if r > max_detour)} "
            f"over-limit pairs:\n  " + "\n  ".join(repeat_lines)
        )
    ...
    # ⚠️ A ROOM WHOSE BUILDING IS UNKNOWN IS NOT A ROOM WITH NO PLACE. the university's own Untis export
        ...
        for left in buildings:
            for right in buildings:
                ...
                if route is None:
                    ...
                    #
                    # ⚠️ AND THE COUNT THAT EXPLAINS IT USED TO PRINT AFTER THIS LINE. The
                    # `unreachable_pairs` tally is computed during routing and was reported at the
                    # very end of `main()`, which this `SystemExit` never reaches — so the failure
                    # named one arbitrary pair by placeholder id and withheld the fact that there
                    # were hundreds of them in two disconnected halves of the path graph. The
                    # numbers are now carried into the message itself.
                    raise SystemExit(
                        f"REFUSING TO WRITE the matrix: no route between {a} "
                        f"({names.get(a, '?')}) and {b} ({names.get(b, '?')}). "
                        f"The generator would treat the gap as an unconstrained transfer.\n"
                        f"{unreachable_pairs} of {len(usable) * (len(usable) - 1) // 2} building "
                        f"pairs have no path between them, which means the pedestrian graph is in "
                        f"more than one piece. Usually the footpath extract is clipped tighter "
                        f"than the campus boxes, so a link that exists on the ground is missing "
                        f"from the data — check the bbox in tools/geodata/fetch_osm_footpaths.py "
                        f"against the campuses in config/aoi/{aoi_id}.json."
                    )
