...
from utm import wgs84_to_utm  # noqa: E402
...
def overpass(query: str, attempts: int = 3, timeout: int = 180) -> dict:
    ...
    for attempt in range(attempts):
        for endpoint in OVERPASS_MIRRORS:
            ...
            # A mirror that accepts the request and then drops it mid-body raises
            # `http.client.RemoteDisconnected` or `IncompleteRead`, neither of which urllib wraps.
            # Without them here the retry loop is bypassed on the most common kind of flake.
            except (
...
def footprint_area_m2(geometry: list[dict]) -> float:
    ...
    pts = [wgs84_to_utm(p["lon"], p["lat"]) for p in geometry]
    if len(pts) < 3:
        return 0.0
    area = 0.0
...
def ownership() -> dict:
    ...
    ⚠️ THE DEFAULT `outlineIds` IS DERIVED BY SPLITTING `anchorSource`, so it can only be computed
    for a site whose anchors ARE single OSM objects. OTH's are; a campus whose anchor is the
    measured centroid of eighty features has no such id, and building the default eagerly turned
    that into an IndexError inside a function whose result was about to be thrown away. So the
    default is computed only when the AOI has not declared one.
    """
    declared = AOI.get("ownership") or {}
    outline_ids = declared.get("outlineIds")
    if outline_ids is None:
        outline_ids = [c["anchorSource"].split()[1] for c in AOI["campuses"]]
    return {
        "siteRelations": [],
        ...
        rows.append(
            {
                ...
                "polygonUtm32": [
                    [round(x, 2), round(y, 2)]
                    for x, y in (wgs84_to_utm(p["lon"], p["lat"]) for p in geom)
                ],
            }
        )
    return rows
...
def main() -> None:
    ...
    # ⚠️ A REGEX, NOT A SUBSTRING — and the two agree for the first two customers, which is why the
    # substring version survived this long. OTH tags one operator string and LMU tags one, so
    # `'ludwig-maximilians' in operator` is exact for both. Münster tags TWO ('Universität Münster'
    # and the older 'Westfälische Wilhelms-Universität'), and a substring can only name one of
    # them: the other half of the university silently stops being university property. Every
    # existing pattern is a plain literal and therefore matches itself as a regex, so nothing
    # about OTH's or LMU's committed building list moves.
    operator_re = re.compile(operator_pattern) if operator_pattern else None
    ...
    # ⚠️ THE LAST-RESORT SIGNAL, AND THE ONLY ONE THAT IS ABOUT THE BUILDING RATHER THAN ITS OWNER.
    # `building=university` says "somebody uses this for higher education"; it does NOT say WHICH
    # university, so it is opt-in per AOI and must stay off wherever two of them share a district.
    # LMU MUST NEVER SET IT: TUM's Stammgelände is inside the LMU core box and is tagged this way
    # throughout, which is the same trap the indoor probe records — an unfiltered count credits LMU
    # with 952 rooms instead of 527.
    #
    # It exists because two of the five sites cannot be recognised without it. FAU's buildings are
    # named 'Chemikum', 'Kollegienhaus', 'Werkstoffwissenschaften' — no university in the name, no
    # operator on the way, and no parcel outline to test against — and the first fetch found 34 of
    # them. Tübingen found 19 in a 3280-building box, missing the Neue Aula and the Kupferbau, i.e.
    # its own focus places. In both towns there is exactly one university, so the tag answers the
    # question it cannot answer in Munich.
    building_tags = {t.lower() for t in (own.get("buildingTags") or [])}
    if building_tags:
        print(f"  building tags accepted as ownership: {', '.join(sorted(building_tags))}")
    ...
    for campus in AOI["campuses"]:
        ...
        for r in rows:
            ...
            by_operator = bool(
                operator_re and operator_re.search((r.get("operator") or "").lower())
            )
            ...
            signals = (
                ...
                ("building tag", (r.get("buildingTag") or "").lower() in building_tags),
            )
