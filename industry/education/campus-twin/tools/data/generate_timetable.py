...
def load_buildings() -> list[dict]:
    ...
    # Through the site rather than straight off disk: `buildings_payload()` translates the campus
    # ids of a site that borrows another's buildings file. See `Site.buildings_payload`.
    payload = SITE.buildings_payload()
...
def building_rows(buildings: list[dict]) -> list[dict]:
    ...
    for b in ordered:
        ...
        easting, northing = wgs84_to_utm(b["lon"], b["lat"])
        rows.append(
            {
                "buildingId": letter,
...
def plan_rooms_by_building(buildings: list[dict]) -> tuple[dict[str, list[dict]], str]:
    ...
    surveyed = SITE.buildings_payload()["buildings"]
...
def cohort_rows(rng: random.Random) -> list[dict]:
    ...
    # ⚠️ A COHORT ID IS BUILT FROM THE FIRST FOUR LETTERS OF THE PROGRAMME, AND FOUR LETTERS ARE
    # NOT ALWAYS ENOUGH. Münster's profile listed both `Chemie` and `Chemieingenieurwesen` under
    # one faculty, so three pairs of cohorts came out sharing an id: their sessions were counted
    # against each other's slots and their attendance against each other's rooms. The dataset was
    # produced without complaint and the symptom appeared four steps downstream as 20-145 hard
    # conflicts, all of them labelled with the colliding id — which reads exactly like a room
    # shortage, and four rounds of tuning went into teaching shares, campus rules and seeds before
    # anyone counted the cohorts. Refusing to build is the same treatment the surname pool gets,
    # for the same reason: an identifier that silently names two things is worse than a crash.
    ids = [r["cohortId"] for r in rows]
    duplicates = sorted({i for i in ids if ids.count(i) > 1})
    if duplicates:
        raise SystemExit(
            f"duplicate cohort ids: {', '.join(duplicates)}\n"
            "Two programmes in one faculty share their first four letters. Rename one of them in "
            f"config/academic/{SITE.id}.json — the id is {{faculty}}-{{programme[:4]}}-{{semester}}."
        )
    return rows
...
def main() -> None:
    ...
    # ⚠️ THE DEFAULT COMES FROM THE SITE'S OWN PROFILE, and only then from this constant. A seed
    # that lives solely in a command line is a seed nobody will type twice: `campus-demo` shares
    # TU Berlin's building stock and has the same faculty/programme/semester shape, so on the
    # shared default seed it generated a dataset with EXACTLY the same row counts — 1 325 sessions,
    # 334 courses, 42 cohorts, 88 teachers. The two datasets were genuinely different (zero shared
    # ids anywhere) and looked like a relabelled copy, and `test_multi_site.py`'s "the universities
    # are not the same data" check fired on the coincidence. Recording the seed in
    # `config/academic/<site>.json` means the difference survives the next regeneration by someone
    # who does not know to pass a flag.
    parser.add_argument(
        "--seed",
        type=int,
        default=None,
        help="override the seed; defaults to the profile's `seed`, else 20260730",
    )
    parser.add_argument(
        "--out",
        default=None,
    ...
    seed = args.seed if args.seed is not None else int(PROFILE.get("seed", 20260730))
    rng = random.Random(seed)
    out_dir = (ROOT / args.out) if args.out else SITE.synth
    out_dir.mkdir(parents=True, exist_ok=True)
    print(f"=== {SITE.label} — {len(FACULTIES)} faculties, {len(BLOCKS)} blocks/day, seed {seed} ===")
