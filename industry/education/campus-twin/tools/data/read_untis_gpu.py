...
# ⚠️ THE INPUT IS A REAL TIMETABLE EXPORT AND IS NOT IN THIS REPOSITORY — restricted under
# config/release.json, lever C. It lives wherever the person running this keeps it, so it is named
# by environment variable and not by path. It used to be one developer's OneDrive folder, which
# made this script unrunnable anywhere else and published that person's directory layout in a
# gallery template. `main()` refuses with the variable's name when the directory is not there.
UNTIS_DIR_ENV = "CAMPUS_UNTIS_DIR"
CUSTOMER = Path(os.getenv(UNTIS_DIR_ENV, ""))
...
def build() -> dict[str, Any]:
    ...
    report["availability"] = {
        ...
        "$sourceValues": "gpu016 = the university's Zeitwünsche export · stvp_excel = a lecturer's own "
...
def main() -> None:
    ...
    if not os.getenv(UNTIS_DIR_ENV):
        raise SystemExit(
            f"Set {UNTIS_DIR_ENV} to the folder holding the Untis export "
            f"(the one containing 'Untis-Export/'). It is restricted data and is not in this repo."
        )
    if not UNTIS.exists():
