...
Output (into public/terrain/<aoi-id>/):
  shell.u16   uint16 little-endian, row-major, row 0 = north
  shell.json  grid, UTM origin, the measured offset, and the core rectangle in shell metres
...
from utm import active_zone, bbox_to_utm, utm_to_wgs84, utm_to_wgs84_array
...
def load_core(aoi_id: str) -> tuple[np.ndarray, dict]:
    ...
    meta = json.loads((directory / "heightmap.json").read_text(encoding="utf-8"))
...
def main() -> None:
    ...
    min_e, min_n, max_e, max_n = bbox_to_utm(*bbox_wsen(cfg, "shell"))
    ...
    lon, lat = utm_to_wgs84_array(grid_e, grid_n)
    ...
    core, core_meta = load_core(cfg["id"])
    ...
    # Shell cell centres, in core-grid coordinates.
    ...
    if ring.sum() < 50:
        raise SystemExit("the overlap ring is too small to measure an offset")

    core_at_ring = core[
        core_row[ring].astype(np.int32), core_col[ring].astype(np.int32)
    ]
    difference = shell[ring] - core_at_ring
    ...
    print(f"\n=== seam offset, measured in a {RING_WIDTH_M:.0f} m ring ({ring.sum()} cells) ===")
    ...
    metadata = {
        ...
        # ⚠️ Derived, not written down — see the same note in `build_terrain.py`. The shell shares
        # the core's grid so it shares the core's zone, and TU Berlin's is 33.
