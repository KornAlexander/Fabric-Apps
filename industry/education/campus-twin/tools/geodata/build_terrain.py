...
Output (into public/terrain/<aoi-id>/):
  heightmap.u16        uint16 little-endian, row-major, north-to-south (image order)
  heightmap_nodata.u8  255 where the cell was filled rather than measured
  heightmap.json       grid size, UTM origin, height scale/offset, focus places, attribution

⚠️ The grid resolution is NOT in the filename. It used to be (`heightmap_4m.*`), and the writer
derived it from the AOI config while seven readers hard-coded the 4 m spelling — so the first AOI
with a different grid built a perfectly good heightmap that nothing could open. The resolution is
already in the metadata, where a reader can act on it; putting it in the filename too only created
a second source of truth that could disagree with the first.
...
Usage
  ...
  python tools/geodata/build_terrain.py --resolution 2
"""
...
from utm import active_zone, bbox_to_utm, utm_to_wgs84, wgs84_to_utm
...
def build(
    ...
    print(f"origin (UTM{active_zone()}): {origin_e} E, {origin_n} N")
    ...
    lon_w, lat_s = utm_to_wgs84(origin_e, origin_n)
    lon_e, lat_n = utm_to_wgs84(origin_e + width * resolution, top_n)
    ...
    def project(entries: list[dict]) -> list[dict]:
        ...
        for entry in entries:
            e, n = wgs84_to_utm(entry["lon"], entry["lat"])
    ...
    # ⚠️ THE ATTRIBUTION IS READ FROM THE AOI, AND USED TO BE A CONSTANT. Every heightmap this
    # pipeline had ever written credited the Bayerische Vermessungsverwaltung under CC BY 4.0 —
    # including Tübingen's, which is the LGL under dl-de/by-2-0, and Aachen's, Köln's and
    # Münster's, which are Geobasis NRW under dl-de/zero-2-0. Five of the ten shipped sites named
    # the wrong survey authority and the wrong licence in the file the app reads, while `NOTICE.md`
    # promised the opposite in as many words: "the app renders that string rather than a constant".
    # It was invisible because the first four AOIs really were Bavarian, and a wrong credit does
    # not look wrong — it looks like a credit. Found when TU Berlin, which is dl-de/zero-2-0,
    # produced a heightmap crediting Bavaria.
    geo = geobasis or {}
    authority = geo.get("authority", "Bayerische Vermessungsverwaltung (LDBV)")

    metadata = {
        "width": width,
        "height": height,
        "resolutionM": resolution,
        # ⚠️ DERIVED, NOT WRITTEN DOWN. This said "EPSG:25832" unconditionally, which was true of
        # every site until TU Berlin and is a lie for it — the grid below really is in zone 33.
        # A consumer trusting this field would place the whole campus several hundred kilometres
        # west. The zone comes from the AOI's `workingCrs`, bound by `load_aoi`.
        ...
        "nodataFill": "nearest",
        "nodataNote": (
            "Cells without DGM1 data — a survey authority's tiles stop at its own state boundary, "
            "and the UTM envelope of a geographic bbox bows outside the bbox at its corners — are "
            "filled from the nearest measured cell for appearance, and flagged in the nodata mask. "
            "See PLAN.md §4.1."
        ),
        "source": f"DGM1, {authority}",
        "sourceAcquisition": geo.get("acquisition", "open geodata, retrieved 2026"),
        "attribution": geo.get(
            "attribution",
            "Datenquelle: Bayerische Vermessungsverwaltung – www.geodaten.bayern.de "
            "[Daten bearbeitet]",
        ),
        "licence": geo.get("licence", "CC BY 4.0"),
    }
...
def main() -> None:
    ...
    parser.add_argument("--name", default=None, help="output basename (default 'heightmap')")
