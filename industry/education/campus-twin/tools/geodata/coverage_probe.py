...
def tiles_for(bbox: dict) -> list[tuple[int, int]]:
    ...
    corners = [
        wgs84_to_utm(bbox["west"], bbox["south"]),
        wgs84_to_utm(bbox["east"], bbox["south"]),
        wgs84_to_utm(bbox["west"], bbox["north"]),
        wgs84_to_utm(bbox["east"], bbox["north"]),
    ]
...
def main() -> None:
    ...
    descriptors = ROOT / "public" / "terrain" / "oth-regensburg"
    drape = json.loads((descriptors / "drape.json").read_text(encoding="utf-8"))
    height = json.loads((descriptors / "heightmap.json").read_text(encoding="utf-8"))
