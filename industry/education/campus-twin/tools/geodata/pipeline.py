...
def in_berlin(aoi: dict) -> bool:
    """The fifth survey authority, added for TU Berlin — and the first outside UTM zone 32.

    Berlin is why `utm.py` stopped hard-coding the zone. Every other authority here publishes in
    EPSG:25832; Berlin publishes EPSG:25833, and its tile FILENAMES encode zone-33 kilometres, so
    the zone is a question of whether a download exists at all rather than of how accurate it is.

    Berlin also splits its two download products across two different grids — 2 km even-parity for
    the terrain, 1 km for the buildings — which is why `fetch_berlin.py` carries the grid on the
    product rather than as a module constant the way the Bavarian and BW fetchers can.
    """
    return "Geoportal Berlin" in _authority(aoi) or "Land Berlin" in _authority(aoi)
...
STEPS: list[Step] = [
    ...
    Step("dgm1-be", ["fetch_berlin.py"], "Geoportal Berlin 1 m terrain tiles", in_berlin),
    ...
    # Berlin publishes a tree cadastre like Bavaria does — and a better one: 40 873 individually
    # surveyed trees over the TU Berlin core, ~100% of them carrying a measured height, against
    # 21 681 from the Bavarian cadastre at OTH. So Berlin takes the cadastre route rather than the
    # nDOM route, and `build_vegetation.py` is reused unchanged: `fetch_trees_berlin.py` writes the
    # same GeoPackage shape the Bavarian fetcher does.
    Step("trees-be", ["fetch_trees_berlin.py"], "Berlin Baumbestand tree cadastre", in_berlin),
    Step("vegetation-be", ["build_vegetation.py"], "tree instances from the Berlin cadastre", in_berlin),
    ...
    Step("lod2-be", ["fetch_berlin.py", "--product", "lod2"], "Geoportal Berlin LoD2 CityGML tiles", in_berlin),
    ...
    # ⚠️ Berlin publishes an ATOM download for the imagery as well, and it must not be used: it is
    # partitioned by district, and `Mitte.zip` alone is 3.2 GB for an AOI that needs ~1.5 km².
    Step("drape-be", ["fetch_dop20_berlin.py"], "DOP20 orthophoto drape, via the Geoportal Berlin WMS", in_berlin),
