...
# ⚠️ OTH-SPECIFIC, AND IT NOW SAYS SO INSTEAD OF CRASHING. The expectations below are OTH's — the
# Friday cascade, the surname that resolves, the seven-block grid. Run under `SCHEDULER_SITE=lmu`
# it died with `KeyError: 'entries'` from a cohort lookup that simply found nobody, which reads as
# a broken calendar rather than a test pointed at the wrong university. That cost a real diagnosis
# once; `test_get_calendar_tool.py` had already learned the same lesson and this is its guard.
SITE = os.getenv("SCHEDULER_SITE", "oth")
if SITE != "oth":
