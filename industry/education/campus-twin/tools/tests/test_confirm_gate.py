...
applied = proposals.apply(store, pid, 1, confirmed_by="planner")
check("confirming creates a draft", applied.get("draftId", "").startswith("draft-"),
      applied.get("draftId", ""))
check("the draft records who confirmed it", applied.get("confirmedBy") == "planner")
...
# week and cannot write one. `find_substitute` was added the same way (§40.4): it reads the plan and
# the availability table and returns NAMES, ranked with a reason each. It moves nothing — assigning
# the substitute is still a move, and a move still goes through the solver and the confirm gate.
check("the tool surface is read-only plus the solver", surface == {
    "detect_conflicts", "explain_infeasibility", "get_affected_sessions",
    "get_plan_overview", "get_calendar", "propose_repairs", "find_substitute",
}, ", ".join(sorted(surface)))
