...
const SITES = {
  ...
  tuebingen: {
    // ⚠️ `rooms: 0` USED TO BE HERE AS "A FACT ABOUT THE SITE, NOT A GAP IN THE CHECK", and it was
    // true when it was written: Tübingen has three mapped indoor rooms city-wide and none in the
    // Altstadt, so there was no interior to verify. What changed is not the survey — that is still
    // three rooms — but what `occupancy` MEANS at a generated site: 4 539 plates laid out inside
    // the real footprints at the real storey counts, badged `generated`, coloured by the plan the
    // solver is working on. The claim is about the PLAN, not about the walls, and it is now
    // checkable. Floors are set roughly a third below the measured build for the same reason as
    // every other entry: this catches a layer that failed to load, not a data update.
    buildings: 4500,
    trees: 8000,
    rooms: 3000,
    withCalendar: 90,
    neverBooked: -1,
    planner: true,
    // ⚠️ NOTHING TO SURVEY HERE, AND THAT IS THE SITE. Three mapped indoor rooms city-wide, none
    // in the Altstadt — so the hero opens onto a generated interior and the check asks whether it
    // HAS one rather than whether it was measured. See the assertion for why both forms exist.
    expectSurveyed: false,
  },

  // ⚠️ THE FOUR TWINS BELOW WERE MISSING FROM THIS TABLE FOR THE SAME REASON GARCHING AND TÜBINGEN
  // ONCE WERE — they had no planner, so nobody added them, and `--aoi aachen` exited "unknown
  // AOI". That is exactly the hole this file's own note above describes as "how a defect shipped",
  // and it was open again the moment the four gained a scheduler. All floors are MEASURED from the
  // build of 2026-08-21 and set roughly a third below it.
  aachen: {
    // Measured: 6 738 LoD2 buildings, 19 454 cadastre trees, 19 015 rooms, 134 with a calendar.
    buildings: 4500,
    trees: 12000,
    rooms: 12000,
    withCalendar: 90,
  ...
  koeln: {
    // Measured: 5 928 LoD2 buildings, 4 063 trees, 8 131 rooms, 156 with a calendar.
    // ⚠️ The tree floor is FAR lower than its neighbours' and that is Köln, not a broken layer:
    // this is the most tightly built core in the repository, one campus of 1132 x 800 m with very
    // little unbuilt ground. Copying Aachen's 12 000 here would fail every run.
    trees: 2500,
    buildings: 4000,
    rooms: 5000,
    withCalendar: 100,
  ...
  muenster: {
    // Measured: 6 224 LoD2 buildings, 9 785 trees, 7 555 rooms, 152 with a calendar.
    buildings: 4000,
    trees: 6000,
    rooms: 5000,
    withCalendar: 100,
  ...
  'tu-berlin': {
    // MEASURED on the first completed build, 2026-08-26: 10 202 LoD2 buildings, 31 566 rendered
    // trees (from 40 613 surveyed), 15 553 rooms, 108 rooms carrying a calendar. Set roughly a
    // third below, like the entries above — these catch a layer that failed to load or a stale
    // asset, not a data update.
    //
    // ⚠️ THESE REPLACE PROVISIONAL FLOORS. Before the build ran this entry carried deliberately
    // low placeholders (6000/4000/5000/100) and said so in as many words, because a floor
    // invented to look plausible is worse than an honest placeholder — it would pass.
    buildings: 7000,
    trees: 20000,
    rooms: 10000,
    withCalendar: 70,
  ...
  'campus-demo': {
    // Beispiel-Universität, the generic site. ⚠️ THE GROUND FLOORS ARE THE SAME NUMBERS AS
    // `tu-berlin` ON PURPOSE, and not because they were copied: this AOI declares
    // `assetsFrom: 'tu-berlin'` and is served the identical buildings and vegetation files, so
    // anything else here would be asserting a difference that cannot exist. The ROOM figures are
    // its own — measured 2026-08-26: 15 552 rooms, 104 with a calendar, from its own timetable.
    buildings: 7000,
    trees: 20000,
    rooms: 10000,
    withCalendar: 70,
  ...
  'fau-erlangen': {
    // Measured: 9 013 LoD2 buildings, 36 072 trees — the largest of either count in the
    // repository — 11 450 rooms, 158 with a calendar.
    buildings: 6000,
    trees: 24000,
    rooms: 7500,
    withCalendar: 100,
...
try {
  ...
  if (hero) {
    ...
    // it says nothing about whether the floors being shown are surveyed or invented, and at OTH
    // and LMU the hero is chosen precisely because its interior is real. This is the assertion
    // that would have caught the join failing silently — the build once reported "0 vermessen,
    // 8796 generiert" while every count above stayed healthy.
    //
    ...
    //
    // ⚠️ AND AT FIVE OF THE EIGHT SITES THERE IS NOTHING TO SURVEY, which is a fact about those
    // universities rather than a defect to assert away. RWTH, Köln, Münster, FAU and Tübingen
    // publish no floor plans and OpenStreetMap has no interiors there, so every room is generated
    // inside the real footprint at the real storey count and `expectSurveyed` is false. What is
    // checked instead is that the building HAS an interior at all — a hero that opens onto zero
    // rooms is the same failure at a generated site as an unsurveyed one is at OTH, and it is the
    // failure that actually happens (an asset that did not load, a hero id that drifted).
    //
    // Asserting `surveyed > 0` everywhere would have meant either a permanently red gate on five
    // sites, or dropping the check for all eight — and the second is how a gate quietly stops
    // covering the sites it was written for.
    const interior = await page.evaluate(async ([aoiId, code]) => {
      const res = await fetch(`/terrain/${aoiId}/rooms.json`);
      if (!res.ok) return { surveyed: -1, total: -1 };
      const meta = await res.json();
      const mine = meta.rooms.filter((r) => r.building === code);
      const perRoom = mine.filter(
        (r) => r.provenance === 'measured' || r.provenance === 'plan'
      ).length;
      if (perRoom > 0) return { surveyed: perRoom, total: mine.length };
      ...
      const surveyed = /measured|survey/i.test(declared) ? mine.length : 0;
      return { surveyed, total: mine.length };
    }, [aoi, hero]);
    if (expect.expectSurveyed === false) {
      check(
        'the opened building has an interior (all generated at this site)',
        interior.total > 0,
        `${interior.total} rooms, ${interior.surveyed} surveyed`
      );
    } else {
      check(
        'the opened building has real room outlines',
        interior.surveyed > 0,
        `${interior.surveyed} real`
      );
    }
  }
