...
/**
 * The load is roughly 47 MB before there is anything to look at, and for most of that time the
...
test('the map says it is loading, and keeps saying how far along it is', async ({ page }) => {
  ...
  // terrain stage, so the declared total grew from 15.6 MB to 17.4 MB and the bar fell back from
  // 100 % to 90 %. Within a stage the percentage must never decrease.
...
test('the controls stay out of the way until there is something to control', async ({ page }) => {
  ...
  // A full timeline panel over an empty canvas — scrubber, play button, layer toggles, all of
  // them inert — is what made the page look broken rather than busy in the first place.
  await expect(page.getByTestId('twin3d-loading')).toBeVisible();
  await expect(page.getByTestId('twin3d-scrubber')).toHaveCount(0);
  await expect(page.getByTestId('twin3d-play')).toHaveCount(0);
