 ...
 *
 * ⚠️ This was 4175, which `Campus-Scheduler` in the same workspace also uses. `reuseExistingServer`
 * does not check *which* application answers on the port, so when that project's dev server was up
 * first, every spec here loaded Campus-Scheduler and timed out waiting for elements that exist in
 * neither app. Eight failures that looked like a regression and were a port collision.
 *
 * A port nothing else in the workspace claims, and an override for when something does.
 */
const PORT = process.env.FLUT_E2E_PORT ?? '4179';
