...
and reports what to do next. Delta conversion is done by a Fabric notebook or the Lakehouse UI's
"Load to Tables"; this script's job is to get the data there reproducibly.

PLAN §8.1: items live in the existing `Rayfin Apps` workspace (§14 Q3) and every item carries the
`Flut-Insights` prefix, because the workspace is shared with the wind twin.
...
LAKEHOUSE_NAME = "FlutInsightsLakehouse"
...
def create_lakehouse(token: str, workspace_id: str, name: str) -> dict:
    status, payload, headers = request(
        ...
        token,
        {"displayName": name, "description": "Flut-Insights — Ahrtal 2021 flood twin (demo, synthetic portfolio)"},
    )
