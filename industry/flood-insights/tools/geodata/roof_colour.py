...
#: The hue band this app's flood ramp owns, and the saturation a roof is allowed inside it.
#:
#: ⚠️ **This constant is specific to Flut-Insights and has no equivalent in the project this module
#: came from.** There, roof colour was the only thing on screen and a saturated blue slate was
#: simply a saturated blue slate. Here, colour on a building is a WATER DEPTH first: the ramp runs
#: pale blue-grey at first contact to indigo when submerged, chosen precisely because nothing else
#: in the scene occupied that family.
#:
#: Measured after the first build, and the collision was not marginal: **63.3 % of Ahr roofs and
#: 81.4 % of Steinbach roofs** landed in hue 0.50–0.75 above s 0.12, and **12.9 % of Steinbach
#: roofs came within reach of the `deep` stop** — a dry slate house wearing the colour of a house
#: under two and a half metres of water. Steinbach's median saturation was 0.312 with a p90 of
#: 0.618, pinned against SATURATION_CAP.
#:
#: The cap corrects an artefact rather than suppressing a measurement. Rhineland and Eifel slate is
#: very nearly neutral in life (s ≈ 0.05–0.15); a roof arriving at 0.6 is SATURATION_GAIN — which
#: exists to undo haze on clay tile, and is multiplicative so it lifts whatever cast it finds —
#: amplifying a small blue bias in a hazy, coarse orthophoto. Hue survives untouched, so a genuinely
#: blue-grey roof still reads blue-grey; it simply may not shout it in the one voice the water uses.
#: Warm roofs are not affected at all.
WATER_HUE_RANGE = (0.50, 0.75)
WATER_BAND_SATURATION_CAP = 0.18
...
def to_albedo(rgb: tuple[int, int, int], population_value: float) -> tuple[int, int, int]:
    ...
    if WATER_HUE_RANGE[0] <= h <= WATER_HUE_RANGE[1]:
        s = min(s, WATER_BAND_SATURATION_CAP)
