# HSL Realtime Insights

Live map of Helsinki public transport, built as a **Microsoft Fabric App** (Rayfin) on top of a
**Real-Time Intelligence** stack in Microsoft Fabric.

> Credit to **Kevin Thomas** for the original Helsinki real-time transit solution. This repository
> is a rebuild of that idea: the same Real-Time Intelligence architecture, and the same Fabric
> portal host bridge for querying the semantic model. Added here are a token-free 3D city twin and
> a fallback chain that lets the app also run outside the portal, where there is no host to hand
> the DAX to.

Vehicle positions come from the public [HSL GTFS-RT feeds](https://hsldevcom.github.io/gtfs_rt/)
(Helsingin seudun liikenne). No API key is required.

## What it does

- Live tram, bus and metro positions from the public HSL feed
- Eventstream into an Eventhouse, a semantic model over KQL, all inside Fabric
- End-user SSO on the Kusto datasource - every user queries as themselves
- An hourly producer notebook that holds no secrets
...
```mermaid
flowchart LR
  HSL["HSL GTFS-RT feeds<br/>vehicle-positions - trip-updates"]
  NB["Notebook<br/>NB_Helsinki_Realtime_Tracker"]
  ES["Eventstream<br/>ES_HSL_Events<br/>(custom endpoint)"]
  EH["Eventhouse HSL_EH<br/>raw_events -> update policies -><br/>vehicle_positions / trip_updates / alerts<br/>+ MV last_vehicle_position"]
  SM["Semantic model HSL_KQL_SM<br/>DirectQuery over Kusto"]
  APP["Fabric App<br/>hsl-realtime-insights<br/>React + Leaflet"]

  HSL -->|"poll 1.2 s / 11 s"| NB --> ES --> EH --> SM -->|"DAX"| APP
```

The app never talks to Kusto directly - it only ever issues DAX against `HSL_KQL_SM`, and the
...
as glaring white boxes - which is exactly what "why are the buildings grey?" looks like from the
outside. The tileset therefore carries a `Cesium3DTileStyle` that multiplies everything by a warm
stone tint (`BUILDING_TINT`). Multiply is the right operator here: it barely touches a textured
facade, and it drops the blank ones into masonry.
...
| `tools/verify_publishable.py` | audits the repo for tenant-specific values before publishing |
| `docs/publishing.md` | what to change to contribute this to awesome-rayfin |
...
Sources and their licences are listed in [NOTICE.md](NOTICE.md).
