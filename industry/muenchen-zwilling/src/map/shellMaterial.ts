...
/**
 * The coarse Munich terrain shell extends beyond the high-resolution city rectangle.
 * Its grid and image dimensions come from the imported metadata; distant detail fades into haze.
 *
...
// The core heightmaps, so the shell can match each core's edge exactly.
//
// ⚠️ TWO OF THEM, AND THAT IS THE WHOLE POINT OF THIS FILE NOW. One shell around one core was
// enough while the app showed a single city box. This world has two cores 28 km apart inside one
// continuous shell, and the camera flies between them — so the shell has to be cut away and
// feathered at both, not at whichever one happened to be passed in.
uniform usampler2D uCoreHeight;
uniform vec2  uCoreHeightSize;
uniform float uCoreHeightMin;
uniform float uCoreHeightScale;

uniform usampler2D uCoreHeightB;
uniform vec2  uCoreHeightSizeB;
uniform float uCoreHeightMinB;
uniform float uCoreHeightScaleB;

// Everything below is in metres relative to the world origin, which is the centre of the FIRST
// core. The second rectangle is offset by the measured distance between the two UTM grids.
uniform vec4  uCoreRect;    // xy = min corner (x, z), zw = size
uniform vec4  uCoreRectB;
/** 1 when only the first core is present, 2 when both are. */
...
float coreElevationClampedB(vec3 world) {
  vec2 local = (world.xz - uCoreRectB.xy) / uCoreRectB.zw;
  vec2 uv = clamp(local, 0.0, 1.0);
  ivec2 texel = ivec2(uv * (uCoreHeightSizeB - 1.0));
  return uCoreHeightMinB + float(texelFetch(uCoreHeightB, texel, 0).r) * uCoreHeightScaleB;
}

/** Signed distance into a core rectangle: positive inside, negative outside. */
float rectDepth(vec3 world, vec4 rect) {
...
/**
 * Distance into the NEAREST core.
 *
 * ⚠️ max, not min. Each rectangle reports how far inside itself the point is, negative when
 * outside, so the larger of the two is the nearer core. Taking the minimum would feather the
 * shell towards whichever core is further away and cut a hole in the wrong place — and with the
 * two cores 28 km apart it would do so over the entire world.
 */
float coreDepth(vec3 world) {
  float depth = rectDepth(world, uCoreRect);
  if (uCoreCount > 1.5) depth = max(depth, rectDepth(world, uCoreRectB));
  return depth;
}
...
void main() {
  ...
  //
  // The feathering follows whichever core is nearer, which is the same core coreDepth measured.
  if (depth > -uBandM) {
    float weight = clamp(1.0 + depth / uBandM, 0.0, 1.0);
    float nearest = (uCoreCount > 1.5 && rectDepth(world, uCoreRectB) > rectDepth(world, uCoreRect))
      ? coreElevationClampedB(world)
      : coreElevationClamped(world);
    elevation = mix(elevation, nearest, weight);
  }
...
uniform sampler2D uShellDrape;
uniform float uHasShellDrape;
...
// Row 0 of every raster in this pipeline is NORTH, so every sampler goes through one flip.
vec2 gridUv(vec2 uv) {
  return vec2(uv.x, 1.0 - uv.y);
}
...
void main() {
  ...
  // The orthophoto, when this site has one for its shell.
  //
  // ⚠️ THE RAMP ABOVE IS INVENTED AND THIS IS NOT. terrainColour() maps height to a pasture →
  // rock palette, which is a reasonable guess in the Alps and a falsehood on the Isar plain: 60 m
  // of relief over 20 km paints Munich, the Isar, the Forstenrieder Park and the airport in one
  // shade of green. The photograph is the same ground the core already shows, sampled about twenty
  // times more coarsely — so the tiers differ in sharpness, which the eye reads as distance,
  // rather than in kind, which it reads as a seam. The ramp stays as the fallback for a site whose
  // shell has not been photographed.
  if (uHasShellDrape > 0.5) {
    vec3 photo = texture(uShellDrape, gridUv(vUv)).rgb;
    colour = photo;

    // ⚠️ Same one-third rule as the core drape, and for the same reason: an orthophoto was taken
    // on a real morning and already carries that day's shadows. Hillshading it again drives north
    // faces to black; dropping the shading entirely flattens the relief into a paper map.
    light = mix(vec3(1.0), light, 0.34);
  }
...
export interface ShellMaterialOptions {
  ...
  /** One or two cores. The first defines the world origin. */
  cores: readonly ShellCore[];
  elevationRangeM: { min: number; max: number };
  /** The coarse orthophoto over the shell. Absent on a site built before it existed. */
  shellDrapeTexture?: THREE.Texture | null;
}

export function createShellMaterial(options: ShellMaterialOptions): THREE.ShaderMaterial {
  const { shell, cores } = options;
  if (cores.length < 1 || cores.length > 2) {
    throw new Error(`The shell supports one or two cores, not ${cores.length}.`);
  }
  const [first, second] = cores;

  // A 1x1 white pixel stands in when there is no photograph, so the sampler is always bound. An
  // unbound sampler2D reads as texture unit 0 — whatever happens to be there — which is how a
  // missing optional texture turns into the heightmap painted across the horizon.
  const shellDrapeFallback = new THREE.DataTexture(new Uint8Array([255, 255, 255, 255]), 1, 1);
  shellDrapeFallback.needsUpdate = true;
  ...
  return new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uCoreHeight: { value: first.texture },
      uCoreHeightSize: { value: new THREE.Vector2(first.meta.width, first.meta.height) },
      uCoreHeightMin: { value: first.meta.heightMinM },
      uCoreHeightScale: { value: first.meta.heightScale },
      uCoreRect: { value: new THREE.Vector4(...first.rect) },
      // With one core the second slot is bound to the first rather than left dangling: an unbound
      // usampler2D reads texture unit 0, and uCoreCount would be the only thing standing between
      // that and a hole cut in the shell at the world origin.
      uCoreHeightB: { value: (second ?? first).texture },
      uCoreHeightSizeB: {
        value: new THREE.Vector2((second ?? first).meta.width, (second ?? first).meta.height),
      },
      uCoreHeightMinB: { value: (second ?? first).meta.heightMinM },
      uCoreHeightScaleB: { value: (second ?? first).meta.heightScale },
      uCoreRectB: { value: new THREE.Vector4(...(second ?? first).rect) },
      uCoreCount: { value: cores.length },
      ...
      uShellDrape: { value: options.shellDrapeTexture ?? shellDrapeFallback },
      uHasShellDrape: { value: options.shellDrapeTexture ? 1 : 0 },
    },
  });
}
