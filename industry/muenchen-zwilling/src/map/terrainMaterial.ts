 ...
 *
 * The city view uses aerial imagery over this measured ground. The palette remains only as
 * the inherited material's fallback; the application requires imagery before reporting ready.
 */
...
void main() {
  ...
  // ⚠️ WORLD SPACE, NOT OBJECT SPACE. This used to be "vWorld = displaced", which was correct
  // while there was exactly one core and it sat on the world origin. With a second core 28 km
  // away the two are no longer the same thing, and the fragment shader measures haze distance
  // from vWorld — so an offset core would haze as though the camera were standing in the middle
  // of it. The model matrix is applied here rather than folded into modelViewMatrix below,
  // exactly as the shell material already does for the same reason.
...
/**
 * The other three quadrants of a tiled drape.
 *
 * ⚠️ A LARGE CORE CANNOT USE ONE TEXTURE. The airfield core is 8.6 km across; at the 4096 px
 * cap that a single drape can safely carry it would be 2.1 m per pixel, and runway markings,
 * taxiway centrelines and stand numbers all disappear. 8192 px is 1.05 m/px — the same sharpness
 * as the city core — but an 8192 px mipmap chain is ~300 MB, which Chromium accepts and then
 * silently samples BLACK. Four 4096 px quadrants give the resolution without the failure.
 */
uniform sampler2D uDrape1;
uniform sampler2D uDrape2;
uniform sampler2D uDrape3;
/** 1 for a single drape, 4 for the 2 x 2 grid. No other value is supported. */
uniform float uDrapeTiles;
/** Half a texel, so bilinear filtering cannot reach across a tile boundary into the next image. */
uniform vec2  uDrapeInset;
uniform float uHasDrape;
uniform float uShowDrape;
/** 1 when the surrounding shell carries the same orthophoto, so the boundary fade is not needed. */
uniform float uShellPhoto;
// Dims the whole surface so something drawn above it becomes the subject. A 20 cm orthophoto is
// gorgeous and extremely busy, and floating floor plates are simply lost against it at full
// brightness — measured, not guessed: exploding a building changed only 0.3 % of the frame.
uniform float uDim;
...
/**
 * The orthophoto, from one texture or from a 2 x 2 grid of them.
 *
 * ⚠️ THE QUADRANT INDEX IS COMPUTED IN ALREADY-FLIPPED SAMPLING SPACE, which is why callers pass
 * gridUv(vUv) in rather than the raw uv. The tiles are named by raster row, row 0 = north, and
 * a flipped sampling coordinate has y = 0 at the north edge too — so index and file agree without
 * either side having to interpret the other's convention.
 *
 * ⚠️ THE SAMPLERS ARE BRANCHED, NOT INDEXED. GLSL ES 3.00 requires a constant expression to index
 * a sampler array, so uDrape[q] does not compile; four explicit reads do.
 *
 * ⚠️ THE INSET IS NOT COSMETIC. Each tile is a separate texture with clamp-to-edge wrapping, so a
 * bilinear tap at a tile boundary reads the edge texel twice instead of the neighbour's first
 * texel. Without the half-texel inset that shows as a faint cross through the middle of the core,
 * running the full length of a runway.
 */
vec3 sampleDrape(vec2 g) {
  if (uDrapeTiles < 2.0) return texture(uDrape, g).rgb;

  vec2 t = g * 2.0;
  vec2 cell = clamp(floor(t), 0.0, 1.0);
  vec2 luv = clamp(t - cell, uDrapeInset, 1.0 - uDrapeInset);

  if (cell.y < 0.5) {
    return cell.x < 0.5 ? texture(uDrape, luv).rgb : texture(uDrape1, luv).rgb;
  }
  return cell.x < 0.5 ? texture(uDrape2, luv).rgb : texture(uDrape3, luv).rgb;
}
...
// already 800 m.
//
// The fallback ramp gets paler with height. Aerial imagery replaces it in the Munich view.
...
// OpenStreetMap land-cover class ids are documented in the imported land-use descriptor.
...
void main() {
  ...
  //
  // ⚠️ UNLESS THE SHELL IS PHOTOGRAPHED TOO, IN WHICH CASE THE FADE IS THE ARTEFACT. The ring
  // exists to let the core arrive at its boundary wearing the same invented colour the shell
  // starts from. Give the shell the same orthophoto and that reasoning inverts: the core now
  // dissolves out of a photograph into a pale hypsometric band, and then back into a photograph —
  // a bright wedge along the boundary that is far more visible than the seam it was avoiding.
  // Measured on the city view when the shell imagery was added.
  float drapeMix = uHasDrape * uShowDrape * mix(edgeFade, 1.0, uShellPhoto);
  if (drapeMix > 0.0) {
    vec3 photo = sampleDrape(gridUv(vUv));
...
export interface TerrainMaterialOptions {
  ...
  /**
   * A 2 x 2 grid of orthophoto quadrants, in raster order (north-west, north-east, south-west,
   * south-east), for a core too large to photograph in one texture.
   *
   * ⚠️ MUTUALLY EXCLUSIVE with `drapeTexture`, and exactly four entries or none. Anything else is
   * rejected rather than silently rendering three quarters of an airfield.
   */
  drapeTiles?: readonly THREE.Texture[] | null;
  /** Pixel size of one tile, used to inset sampling by half a texel at the tile seams. */
  drapeTilePx?: { width: number; height: number } | null;
  /**
  ...
  /**
   * Whether the shell around this core carries the same orthophoto.
   *
   * When it does, the drape must NOT be faded out at the boundary: the fade exists to reach the
   * shell's invented colour, and against a photographed shell it draws a pale band instead.
   */
  shellPhoto?: boolean;
}
...
export function createTerrainMaterial(options: TerrainMaterialOptions): THREE.ShaderMaterial {
  ...
  const tiles = options.drapeTiles ?? null;
  if (tiles && tiles.length !== 4) {
    throw new Error(`A tiled drape must have exactly 4 quadrants, not ${tiles.length}.`);
  }
  if (tiles && options.drapeTexture) {
    throw new Error('A core has either one drape or four quadrants, never both.');
  }
  const drape0 = tiles ? tiles[0] : (options.drapeTexture ?? drapeFallback);
  const tilePx = options.drapeTilePx ?? { width: 4096, height: 4096 };
  ...
  return new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uDrape: { value: drape0 },
      uDrape1: { value: tiles ? tiles[1] : drapeFallback },
      uDrape2: { value: tiles ? tiles[2] : drapeFallback },
      uDrape3: { value: tiles ? tiles[3] : drapeFallback },
      uDrapeTiles: { value: tiles ? 4 : 1 },
      uDrapeInset: {
        value: new THREE.Vector2(0.5 / tilePx.width, 0.5 / tilePx.height),
      },
      uHasDrape: { value: tiles || options.drapeTexture ? 1 : 0 },
      uShowDrape: { value: 1 },
      uShellPhoto: { value: options.shellPhoto ? 1 : 0 },
      uDim: { value: 1 },
