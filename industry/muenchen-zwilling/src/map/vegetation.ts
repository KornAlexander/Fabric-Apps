...
/**
 * Munich tree locations, ground levels and heights come from the Bavarian survey data.
 * Crown radius is estimated and species are unknown, so the neutral crown is used here.
 * The reusable loader can also interpret explicitly classified crown forms in other datasets.
 *
 * Three things keep the instanced trees affordable:
 *
 ...
 * it is. Drawing every tree as a conifer would make a species claim the data cannot support.
 *
...
export async function loadVegetation(
  ...
  const meta = await reader.json('vegetation.json') as VegetationMeta;
  if (!Number.isSafeInteger(meta.count) || meta.count < 1 || meta.count > 1_000_000 || meta.stride !== 9) throw new Error('Invalid tree metadata.');
  const tracker = new StageTracker('vegetation', 4, report);
  tracker.addExpected(reader.size('vegetation.bin'));
  const bytes = await reader.bytes('vegetation.bin', tracker.track());
  if (!bytes || bytes.byteLength !== meta.count * meta.stride) throw new Error('Required tree payload shape mismatch.');
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Unit-height crown geometry uses the measured height without enlargement.
